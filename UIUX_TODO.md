...
- [~] **[P0][L]** Merge "My Business" + "My Team" + "Business Settings" into one "Setup" tab — **Phase 1 DONE 2026-06-03** (`a8eb438`). New `SetupView` hosts Services/Resources/Team/Working Days/Who Can Do What/Team Access/Business Settings as sub-tabs; orphaned MyBusinessView/MyTeamView deleted; legacy `?tab=` URLs redirect to `?tab=setup&subtab=…`. Verified: tsc clean, 762 unit, affected e2e green. **Phase 2 DONE 2026-06-03.** 2a (`ad1f675`): removed the duplicate Resources editor from BusinessSettingsView — Setup → Resources sub-tab is canonical. 2b (this pass): SettingsView stripped to super-admin onboarding only (~1080 lines removed); its owner-mode Calendar/CRM/Resources was a latent dup reachable only via a stale `?tab=settings` — non-super-admins now get a pointer to Setup. Connections (Calendar+CRM) lives once, in the Business Settings sub-tab under Setup. Verified: tsc + lint clean, 738 unit, e2e green. **IA merge (Phase 1 + 2) fully complete.** — _source: /ux-expert simplification review_
...
- [x] **[P0][S]** Landing demo link → replaced with "Call (630) 937-9478 — try it live" `tel:` link — DONE 2026-05-28 (number was current at time of UX item; subsequently decommissioned; live number is now +1 630-866-1960 per `CLAUDE.md` / `HANDOFF.md`)
...
- [~] **[P2][M]** Cluster C — overlay Escape/focus-trap/backdrop: **behavior DONE** 2026-05-28 (verified 2026-06-03). All 8 surfaces have correct modal behavior via the shared `useFocusTrap` hook (5 import it directly: `FirstRunTour`, `WizardModeChooser`, `WizardWelcome`, `SetupWizard/index`, `EmployeeDayFocusPanel`; the other 3 — `AppointmentPopover`, `StaffProfileCard`, `SkillMapFixPanel` — got inline trap/role/X per docs/TODO.md:139). `useFocusTrap` IS the shared abstraction. The further migration onto the shared Modal _component_ is a behavior-neutral DRY refactor with an 8-file blast radius and zero user-facing change — **deferred as low-value**; reopen only if Modal gains features these overlays need. — _source: docs/TODO.md Cluster C_
