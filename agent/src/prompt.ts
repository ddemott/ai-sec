...
export function buildSystemPrompt(ctx: PromptContext): string {
  ...
  const callerLine = ctx.callerPhone
    ...
    : hasVerification
      ...
      : `You do NOT have the caller's number (blocked/withheld caller ID, or the call was forwarded in — so the caller-ID was the forwarding line, not theirs). Collect BOTH their name AND a good callback number verbally, read the number back to confirm (see the phone-capture rules below), then call identify_caller(name, phone) to save them to the address book, and use that number for any booking. If they can't give a number, offer to take a message.\n\nIMPORTANT: identify_caller may come back with \`requires_verification: true\`. **You have NO way to verify a number on this call, and you must NOT mention verification, codes, or texts.** It means only that you cannot look up an EXISTING account for a number the caller merely spoke — which is right: you should not read a stranger's name and history aloud to whoever claims their number. It does NOT stop you booking. Treat them as a new caller, use the name and number they gave you, and BOOK NORMALLY. A booking reveals nothing about anyone; the caller supplies every fact in it.`;
