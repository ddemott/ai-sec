...
export function buildTools(
  ...
  // call send_verification_code when send_verification_code is not in the toolset is
  // the exact prompt/tool mismatch capability-gating exists to prevent (GH #113): the
  // model tries to call a tool that does not exist, errors, and the caller hears dead
  // air. Removing the tool is not enough — nothing may still POINT AT it.
