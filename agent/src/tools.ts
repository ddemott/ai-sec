...
export function buildTools(
  ...
  return {
    ...
    reschedule_appointment: llm.tool({
      description:
        "Move one of the caller's upcoming appointments to a new date and time. ALWAYS call get_my_appointments first so the caller can pick which appointment to move. Get available slots with get_available_slots if needed. Confirm the new time verbally, then call this. If the slot is taken, offer alternatives.",
      parameters: {
        type: 'object',
        properties: {
          appointment_id: {
          ...
          new_start_time: {
            type: 'string',
            description: 'New start time in ISO 8601 format (e.g. 2026-06-20T14:00:00-05:00).',
          },
          new_end_time: {
            type: 'string',
            description: 'New end time in ISO 8601 format. Match the original appointment duration.',
          },
        },
