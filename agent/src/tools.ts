...
export function buildTools(
  ...
  const allTools: llm.ToolContext = {
    get_customer_context: llm.tool({
      description:
        "Look up a caller in the CRM by phone. Returns the customer's name, a short history, any saved preferences (preferred staff, last service, likes), and sms_consent — whether they have ALREADY agreed to appointment texts (if true, never ask for that permission again). Pass the phone number the caller gave you verbally when you have it; otherwise it falls back to the caller-ID phone. Use this to recognize returning callers.",
      parameters: {
        type: 'object',
        properties: {
          phone: {
    ...
    record_sms_consent: llm.tool({
      description:
        'Record that the caller VERBALLY agreed to receive SMS appointment confirmations and reminders. Do NOT call this if sms_consent was already true — their permission is on file and does not expire. Call this ONLY after you have (1) asked permission, naming the business, (2) said it is for appointment messages only, (3) said "message and data rates may apply", (4) said they can reply STOP anytime — AND the caller clearly said yes. NEVER use this for marketing or promotions; appointment confirmations/reminders only. Pass the mobile number the caller confirmed for texts.',
      parameters: {
        type: 'object',
        properties: {
          phone: {
