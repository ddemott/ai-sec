...
describe('classifyCallOutcome (failsafe WHY classifier)', () => {
  it('returns null for an empty transcript without calling OpenAI', async () => {
    ...
    expect((await classifyCallOutcome('  ', KEY)).outcome).toBeNull();
  ...
  it('returns null when no API key is provided', async () => {
    const fetchSpy = vi.fn();
    vi.stubGlobal('fetch', fetchSpy);
    expect((await classifyCallOutcome('Caller: hi', '')).outcome).toBeNull();
    expect(fetchSpy).not.toHaveBeenCalled();
  });
  ...
  it('returns null for "unclear" (model declined to classify → stays abandoned)', async () => {
    mockReply('unclear');
    expect((await classifyCallOutcome('Caller: ...click', KEY)).outcome).toBeNull();
  });

  it('returns null for an out-of-vocabulary word', async () => {
    mockReply('banana');
    expect((await classifyCallOutcome('Caller: hi', KEY)).outcome).toBeNull();
  });
  ...
  it('returns null (never throws) when fetch rejects', async () => {
    ...
    await expect(classifyCallOutcome('Caller: hi', KEY)).resolves.toEqual({ outcome: null });
  });
});
