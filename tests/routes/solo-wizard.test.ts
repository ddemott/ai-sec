...
describe('Solo Wizard: team_size on tenants + is_personal on resources', () => {
  ...
  it('team_size should default to 1', async () => {
    if (!dbAvailable) return;
    const res = await client.query(`SELECT team_size FROM tenants WHERE tenant_id = $1`, [
      tenantId,
    ]);
  ...
  it('team_size can be updated', async () => {
    if (!dbAvailable) return;
    await client.query(`UPDATE tenants SET team_size = 5 WHERE tenant_id = $1`, [tenantId]);
    const res = await client.query(`SELECT team_size FROM tenants WHERE tenant_id = $1`, [
      tenantId,
    ]);
  ...
  it('is_personal should default to false on resources', async () => {
    if (!dbAvailable) return;
    const res = await client.query(`SELECT is_personal FROM resources WHERE resource_id = $1`, [
      resourceId,
    ]);
  ...
  it('solo setup flow: create tenant with team_size=1, auto-create employee + personal resource + assign services', async () => {
    ...
    const tenant = await client.query(`SELECT team_size FROM tenants WHERE tenant_id = $1`, [
      soloTenantId,
    ]);
    expect(tenant.rows[0].team_size).toBe(1);

    const employees = await client.query(`SELECT * FROM employees WHERE tenant_id = $1`, [
      soloTenantId,
    ]);
    expect(employees.rows.length).toBe(1);

    const resources = await client.query(
      `SELECT * FROM resources WHERE tenant_id = $1 AND is_personal = true`,
      [soloTenantId]
    );
