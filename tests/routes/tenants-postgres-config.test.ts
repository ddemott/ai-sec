...
import { PostgresTenantConfigService } from '../../src/services/tenants/index.js';
import { getRootClient, createTenant, ROOT_DB_URL, skipIfDbDown } from '../utils';
...
describe('PostgresTenantConfigService against real schema', () => {
  ...
  it('HAPPY: getTenantConfig maps every column the service exposes', async () => {
    ...
    await root.query('UPDATE tenants SET owner_phone = $2 WHERE tenant_id = $1', [
      tenantId,
      '+15551234567',
    ]);
  ...
  describe('cache behavior', () => {
    it('HAPPY: a second lookup within TTL hits the cache, not the DB', async () => {
      ...
      await root.query('UPDATE tenants SET name = $2 WHERE tenant_id = $1', [
        tenantId,
        'Mutated-Out-Of-Band',
      ]);
