...
describe('CRUD Routes - Database Level', () => {
  ...
  describe('Employees', () => {
    ...
    it('should isolate employees by tenant', async () => {
      ...
      const res1 = await client.query('SELECT * FROM employees WHERE tenant_id = $1', [tenantId]);
      const res2 = await client.query('SELECT * FROM employees WHERE tenant_id = $1', [tenant2Id]);
  ...
  describe('Resources', () => {
    it('should list resources for a tenant', async () => {
      ...
      const res = await client.query('SELECT * FROM resources WHERE tenant_id = $1 ORDER BY name', [
        tenantId,
      ]);
    ...
    it('should update resource name and description', async () => {
      ...
      const res = await client.query('SELECT * FROM resources WHERE resource_id = $1', [
        resourceId,
      ]);
    ...
    it('should soft-deactivate resource via is_active flag', async () => {
      ...
      await client.query('UPDATE resources SET is_active = false WHERE resource_id = $1', [
        resourceId,
      ]);

      const res = await client.query('SELECT is_active FROM resources WHERE resource_id = $1', [
        resourceId,
      ]);
  ...
  describe('Services', () => {
    ...
    it('should update service', async () => {
      ...
      const res = await client.query(
        ...
        ['New Svc', 'Updated desc', 45, 75.0, svcId]
      );
      ...
      expect(parseFloat(res.rows[0].price)).toBeCloseTo(75.0);
    });
  ...
  describe('Service Mappings', () => {
    ...
    it('should assign employee to service', async () => {
      ...
      const res = await client.query('SELECT * FROM service_employee WHERE tenant_id = $1', [
        tenantId,
      ]);
    ...
    it('should assign resource to service', async () => {
      ...
      const res = await client.query('SELECT * FROM service_resource WHERE tenant_id = $1', [
        tenantId,
      ]);
      expect(res.rows).toHaveLength(1);
      expect(res.rows[0].service_id).toBe(serviceId);
  ...
  describe('Skills', () => {
    ...
    it('should list skills for tenant ordered by name', async () => {
      ...
      await client.query("INSERT INTO tenant_skills (tenant_id, name) VALUES ($1, 'flat-repair')", [
        tenantId,
      ]);
      await client.query("INSERT INTO tenant_skills (tenant_id, name) VALUES ($1, 'alignment')", [
        tenantId,
      ]);
    ...
    it('should delete skill', async () => {
      ...
      await client.query("INSERT INTO tenant_skills (tenant_id, name) VALUES ($1, 'to-delete')", [
        tenantId,
      ]);

      await client.query('DELETE FROM tenant_skills WHERE tenant_id = $1 AND name = $2', [
        tenantId,
        'to-delete',
      ]);
    ...
    it('should reject duplicate skill name within same tenant', async () => {
      ...
      await expect(
        client.query("INSERT INTO tenant_skills (tenant_id, name) VALUES ($1, 'unique-skill')", [
          tenantId,
        ])
      ).rejects.toThrow();
    });
    ...
    it('should allow same skill name for different tenants', async () => {
      ...
      await client.query("INSERT INTO tenant_skills (tenant_id, name) VALUES ($1, 'shared-name')", [
        tenantId,
      ]);
      await client.query("INSERT INTO tenant_skills (tenant_id, name) VALUES ($1, 'shared-name')", [
        tenant2Id,
  ...
  describe('Error diagnostics', () => {
    ...
    it('should reject resource insert with NULL name via NOT NULL constraint', async () => {
      ...
      try {
        await client.query('INSERT INTO resources (tenant_id, name) VALUES ($1, $2)', [
          tenantId,
          null,
        ]);
        expect.fail('Should have thrown a NOT NULL violation');
    ...
    it('should include constraint name in duplicate skill error', async () => {
      ...
      await client.query("INSERT INTO tenant_skills (tenant_id, name) VALUES ($1, 'dup-test')", [
        tenantId,
      ]);
