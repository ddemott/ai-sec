...
import {
  ...
  updateBooking,
  cancelBooking,
} from '../../src/services/crm/squareClient';
...
describe('verifyState', () => {
  it('returns tenantId for a valid state JWT', () => {
    ...
    setSquareEnv();
    const state = jwt.sign({ tenantId: TENANT_ID, purpose: 'square-oauth' }, JWT_SECRET, {
      expiresIn: '10m',
    });
  ...
  it('returns null for a token signed with a different secret', () => {
    ...
    setSquareEnv();
    const state = jwt.sign({ tenantId: TENANT_ID, purpose: 'square-oauth' }, 'wrong-secret', {
      expiresIn: '10m',
    });
    expect(verifyState(state)).toBeNull();
  });
...
describe('exchangeCodeForTokens', () => {
  ...
  it('throws when Square is not configured', async () => {
    ...
    await expect(exchangeCodeForTokens('auth-code')).rejects.toThrow('Square not configured');
  });
});
...
describe('refreshAccessToken', () => {
  ...
  it('throws when Square is not configured', async () => {
    ...
    await expect(refreshAccessToken('old-refresh-token')).rejects.toThrow('Square not configured');
  });
});
...
describe('apiRequest', () => {
  ...
  it('throws when HTTP response is not ok (401)', async () => {
    ...
    await expect(apiRequest('GET', '/customers', 'my-token')).rejects.toThrow(
      'Square API GET /customers failed (401)'
    );
  });
  ...
  it('throws when HTTP response is not ok (429)', async () => {
    ...
    await expect(apiRequest('GET', '/customers', 'my-token')).rejects.toThrow(
      'Square API GET /customers failed (429)'
    );
  });
  ...
  it('throws when HTTP response is not ok (500)', async () => {
    ...
    await expect(apiRequest('GET', '/customers', 'my-token')).rejects.toThrow(
      'Square API GET /customers failed (500)'
    );
  });
  ...
  it('throws on network error', async () => {
    ...
    await expect(apiRequest('GET', '/customers', 'my-token')).rejects.toThrow('network error');
  });
});
...
describe('verifyWebhookSignature', () => {
  ...
  function computeSignature(b: string, url: string, key: string): string {
    const payload = url + b;
  ...
  it('returns false for invalid signature (tampered body)', () => {
    ...
    expect(verifyWebhookSignature(body, signature, signatureKey, notificationUrl)).toBe(false);
  });
  ...
  it('returns false for completely invalid signature', () => {
    ...
    expect(verifyWebhookSignature(body, 'not-valid', signatureKey, notificationUrl)).toBe(false);
  });
  ...
  it('returns false for different length signature (timingSafeEqual catch)', () => {
    ...
    expect(verifyWebhookSignature(body, '', signatureKey, notificationUrl)).toBe(false);
  });
});
...
describe('createCustomer', () => {
  it('POSTs to customers endpoint with data', async () => {
    ...
    const result = await createCustomer('my-token', {
      given_name: 'New',
