-- ============================================================================
-- Thinking Hammer LLC — "Beth" production setup (idempotent, run-once-safe)
-- ============================================================================
-- Tenant: d5e3c6a1-7b9f-4e2a-bf30-8c11a5d8e9f0
--
-- What this does:
--   1. Renames the tenant to "Thinking Hammer LLC" + installs Beth's persona
--   2. Removes the deprecated DynaTire tenant
--   3. Marks the super-admin tenant onboarding_completed (no wizard)
--   4. Seeds the answering-service booking model so Beth can actually book:
...
-- check — which would let Beth book 3am meetings. Gating every service on
...
--   curl .../agent-tools/tenant-config  -> returns Beth's system_prompt
...
-- ----------------------------------------------------------------------------
-- 1. Tenant rename + Beth persona (becomes the agent prompt's identity section;
...
UPDATE tenants SET
  ...
  system_prompt = $BETH$You are Beth, Dale's personal AI Assistant at Thinking Hammer LLC ("Software That Thinks"). You answer Dale's business line with a warm, calm, classy, professional manner. You speak naturally, like a real assistant — never robotic.

# How you open every call
On your very first turn, in one smooth greeting:
1. Introduce yourself: "Hello, my name is Beth — I'm Dale's AI Assistant."
...
- Goal: book a meeting so Dale can discuss the role.
...
# Booking
Dale takes meetings Monday through Friday, with meetings ending by 5 o'clock. Use the availability and booking tools to find and book a real open slot — never promise a time you haven't checked. The services you can book are the Programming Consultation, the SecretaryHQ Demo, and a Personal Callback.$BETH$
