-- BUG-064: Return specific error codes for booking failures
-- Instead of generic "No available resource/employee combination found",
-- return specific codes: TIMESLOT_OCCUPIED, NO_SKILLED_EMPLOYEE, NO_RESOURCES, etc.
...
BEGIN
    -- ═══════════════════════════════════════════════════════════════
    -- STEP 0: Get tenant timezone (CRITICAL FIX for BUG-001)
    -- ═══════════════════════════════════════════════════════════════
    ...
    ELSIF p_window_from IS NOT NULL AND p_window_to IS NOT NULL THEN
        -- Search for earliest available slot in window
    ...
    -- FIXED: Use tenant timezone instead of hardcoded 'UTC'
    ...
    -- ═══════════════════════════════════════════════════════════════
    -- STEP 3b: Diagnose specific failure reason if not found
    -- ═══════════════════════════════════════════════════════════════
    IF NOT v_found THEN
        -- Check if ANY employee with required skills exists
        IF array_length(p_required_skills, 1) IS NOT NULL AND array_length(p_required_skills, 1) > 0 THEN
            SELECT EXISTS(
            ...
            -- Check if skilled employee is scheduled for this day/time
            SELECT EXISTS(
                SELECT 1 FROM employees emp
                INNER JOIN employee_shifts es ON es.employee_id = emp.id
            ...
            -- Check if employee is occupied at this time
            SELECT EXISTS(
                SELECT 1 FROM employees emp
                INNER JOIN employee_shifts es ON es.employee_id = emp.id
        ...
        -- Generic failure (shouldn't reach here, but just in case)
        RETURN QUERY SELECT FALSE, NULL::UUID, NULL::UUID, NULL::TEXT, NULL::UUID, NULL::TEXT,
            NULL::TIMESTAMPTZ, NULL::TIMESTAMPTZ, v_customer_id,
...
-- Document the new error codes
COMMENT ON FUNCTION book_with_scheduling_atomic IS 
'Atomic booking with scheduling. Returns specific error codes:
- TIMESLOT_OCCUPIED: Time slot already booked
- NO_SKILLED_EMPLOYEE: No employee has required skills
- EMPLOYEE_NOT_SCHEDULED: Employee not working at requested time
- NO_AVAILABILITY: Generic failure
