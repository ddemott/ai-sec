-- Pilot #11 of the PK naming convention conversion (per CODING_STANDARDS.md).
-- Renames employees.id → employee_id.
--
-- 3 inbound FKs (appointments.employee_id, employee_schedule.employee_id,
-- service_employee.employee_id) — all already correctly named. Rename
-- closes the asymmetry so `employees.employee_id = appointments.employee_id`
-- can use `USING (employee_id)`.
--
-- RPCs that reference employees.id (all need recreation):
--   book_appointment_atomic — 2 references (assignment_id existence
--     check, skills lookup).
--   book_with_scheduling_atomic — many references (CROSS JOIN pair scan,
--     preferred-employee ORDER BY, fallback EXISTS checks).
--   get_customer_context_for_call — 1 reference (LEFT JOIN in
--     upcoming-appointments aggregate).
--   check_coverage_gaps — 2 references (skill-coverage CTE).
--
-- auto_version_trigger CASE adds 'employees' → 'employee_id'.
-- fn_audit_trigger does NOT fire on employees (only on appointments /
-- customers / resources) so no change needed there.
-- get_effective_shifts / get_effective_shifts_bulk reference
...
-- ────────────────────────────────────────────────────────────────────
-- 1. book_appointment_atomic — 2 e.id substitutions
-- ────────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────────
-- 2. book_with_scheduling_atomic — emp.id → emp.employee_id throughout
-- ────────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────────
-- 3. get_customer_context_for_call — single e.id substitution
-- ────────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────────
-- 4. check_coverage_gaps — e.id → e.employee_id in skill-coverage CTE
-- ────────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────────
-- 5. check_availability_with_tz — single emp.id substitution
-- ────────────────────────────────────────────────────────────────────
...
BEGIN
    ...
    SELECT EXISTS (
        SELECT 1 FROM employees emp
        JOIN employee_schedule sch
            ON sch.employee_id = emp.employee_id
            AND sch.tenant_id = p_tenant_id
...
-- ────────────────────────────────────────────────────────────────────
-- 6. auto_version_trigger — add employees → employee_id
-- ────────────────────────────────────────────────────────────────────
