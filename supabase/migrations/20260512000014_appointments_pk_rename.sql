-- Renames appointments.id → appointment_id.
--
-- Pilot #14 of the PK naming convention conversion (per CODING_STANDARDS.md).
-- Largest blast-radius pilot to date — `appointments.id` is referenced
-- from:
--   - 3 inbound FK rows: calendar_sync (PK), voice_sessions (nullable
--     FK), reminder_schedules (NOT NULL FK with CASCADE). Each carries
--     `appointment_id UUID REFERENCES appointments(id)`; the rename
--     closes the asymmetry so JOINs become `USING (appointment_id)`.
--   - 3 RPCs touch the PK and need recreation: book_appointment_atomic,
--     book_with_scheduling_atomic, get_customer_context_for_call. The
--     first two RETURN INTO `v_new_appointment_id` from
--     `INSERT … RETURNING id`. The third SELECTs `a.id` into a JSONB
--     output.
--   - Both audit/version triggers fire on appointments. Extend the
--     CASE TG_TABLE_NAME mapping in both:
--       - auto_version_trigger (introduced 20260512000009/010)
--       - fn_audit_trigger (introduced 20260512000011)
--
-- The booking RPCs already RETURN TABLE(success, appointment_id, ...) —
-- the *output column name* of the RPC was already `appointment_id` —
-- so external consumers of the RPC result are unchanged. Only the
-- internal `RETURNING id INTO v_…` switches to the new column name.
--
-- `get_customer_context_for_call` returns a JSONB with `'id': a.id`
-- inside `upcoming_appointments[]`. The JSONB key `'id'` is preserved
...
-- ────────────────────────────────────────────────────────────────────
-- 1. book_appointment_atomic — RETURNING id → RETURNING appointment_id
-- ────────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────────
-- 2. book_with_scheduling_atomic — RETURNING id → RETURNING appointment_id
-- ────────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────────
-- 3. get_customer_context_for_call — a.id → a.appointment_id in SELECT.
--    JSONB output key `'id'` preserved (consumer-facing contract).
-- ────────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────────
-- 4. update_appointment_customer (9-arg appointment-only overload) —
--    rewrite `WHERE id` references on `appointments` to
--    `WHERE appointment_id`. The 11-arg polymorphic overload and the
--    INTEGER-employee_id overload also exist on disk from prior
--    migrations but are not exercised by tests and have other latent
--    issues (INTEGER vs UUID for employees) — leaving them as
--    documented dead code per "test or delete" until a real caller
--    surfaces.
-- ────────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────────
-- 5. auto_version_trigger CASE — add appointments → appointment_id
-- ────────────────────────────────────────────────────────────────────
-- appointments is a versioned table (CREATE TRIGGER appointments_auto_version,
-- 20260409100000). Without this branch, after the column rename the
-- trigger would read (to_jsonb(NEW) ->> 'id') and produce NULL record_id
-- rows on every appointment INSERT/UPDATE/DELETE.
...
-- ────────────────────────────────────────────────────────────────────
-- 5. fn_audit_trigger CASE — add appointments → appointment_id
-- ────────────────────────────────────────────────────────────────────
-- appointments has an audit trigger (trg_audit_appointments,
-- 20260316400000). The CASE was added in the resources pilot
-- (20260512000011) with only `resources → resource_id`. Extend.
