-- Migration: appointments.service_id + mapping-table-aware skill check
-- ─────────────────────────────────────────────────────────────────────
-- Background. The dashboard's UI alignment filter (shipped 2026-05-07,
-- commit 967402d) uses the `service_employee` and `service_resource`
-- mapping tables as the source of truth for "who can do what." Pick
-- service X in QuickBookPanel and the Tech dropdown narrows to employees
-- with a (X, employee_id) row. But `book_appointment_atomic` was checking
-- a different model — the `services.required_skills` text array against
-- `employees.skills` text array — and those two models can drift. Worse,
-- the seed data populates `service_employee` rows but does NOT populate
-- `employees.skills`, so passing `p_service_id` from the dashboard would
-- have caused the array check to falsely reject every booking.
--
-- This migration converges the two layers onto the mapping tables:
--   1. Add appointments.service_id (nullable UUID FK) so future bookings
--      record which service they fulfilled — useful for analytics, and
--      lets a future "what did Mike do today" query work without
--      free-text matching on description.
--   2. Update book_appointment_atomic so when p_service_id is provided
--      AND the mapping table has rows for that service, the mapping IS
--      the constraint. The legacy array-based check still runs as a
--      fallback for tenants that haven't populated the mapping.
--
-- Backward-compat. Existing callers that don't pass p_service_id get
-- unchanged behavior (skill check is skipped entirely, just like
-- before). The dashboard's `/appointments/create` will be updated in
-- the next step to thread serviceId through; the agent's
-- `/agent-tools/book-appointment` already passes `p_service_id` (was
-- relying on the array check, which silently missed because employees'
-- skills arrays were empty — now it'll consult the mapping correctly).
...
'Optional FK to the service this appointment fulfills. Nullable for
...
-- ─────────────────────────────────────────────────────────────────────
-- 2. book_appointment_atomic — mapping-aware skill + resource check
-- ─────────────────────────────────────────────────────────────────────
-- Diff from the version in 20260501000001:
--   * After resolving v_employee_id and reading required_skills, FIRST
--     check `service_employee` mapping if rows exist. Only fall back to
--     the array check when the mapping table has NO rows for this
--     service (open-service semantic — matches the UI helper's
--     filterEmployeesByService fall-open branch).
--   * Same for resources via `service_resource` mapping.
--   * Persist service_id on the INSERT when provided.
...
'Atomic appointment booking with full alignment enforcement.
