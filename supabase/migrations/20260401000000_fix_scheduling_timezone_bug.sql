-- Migration: Fix timezone bug in book_with_scheduling_atomic
-- 
-- The function book_with_scheduling_atomic (created 2026-03-24) reintroduced BUG-001
-- by using hardcoded 'UTC' timezone for shift validation instead of tenant timezone.
-- 
-- This causes bookings to fail for tenants in non-UTC timezones because:
-- 1. Day-of-week is calculated wrong (e.g., Friday 9 PM CDT → Saturday 3 AM UTC)
-- 2. Time comparisons fail (comparing UTC time against local shift hours)
--
-- Fix: Use tenant.timezone for all shift validation, just like book_appointment_atomic
...
COMMENT ON FUNCTION book_with_scheduling_atomic IS 
  'Atomically find and book an appointment with timezone-aware shift validation. 
   Fixed 2026-04-01: Use tenant timezone instead of hardcoded UTC (BUG-001 regression).';
