-- Renames customers.id → customer_id.
--
-- Pilot #15 of the PK naming convention conversion (per CODING_STANDARDS.md).
-- Second-most-entangled pilot (after appointments). Customers has:
--   - 4 inbound FKs: appointments.customer_id (CASCADE), audit_log.customer_id
--     (the audit table's record_id is TEXT not FK, but logically), leads.customer_id,
--     reminder_schedules.customer_id, voice_sessions.customer_id (SET NULL).
--     All FK columns are already named `customer_id` — only the SOURCE side
--     (`customers.id`) is asymmetric and needs renaming.
--   - RPCs that SELECT customers.id INTO v_customer_id or UPDATE customers
--     WHERE id = v_customer_id: book_appointment_atomic, book_with_scheduling_atomic,
--     get_customer_context_for_call, update_appointment_customer (both 9-arg
--     and 11-arg overloads).
--   - Both audit/version triggers fire on customers (CREATE TRIGGER
...
-- ────────────────────────────────────────────────────────────────────
-- 1. book_appointment_atomic — SELECT id INTO v_actual_customer_id +
--    RETURNING id INTO v_actual_customer_id both switch to customer_id.
-- ────────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────────
-- 2. book_with_scheduling_atomic — same customers.id → customer_id pattern
-- ────────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────────
-- 3. get_customer_context_for_call — v_customer.id → v_customer.customer_id
--    in the JSON-output building. The JSONB output key `'id'` is
--    preserved as the consumer-facing contract (agent reads it).
-- ────────────────────────────────────────────────────────────────────
...
--    only the trailing `UPDATE customers SET … WHERE id =` switches to
--    `WHERE customer_id =`.
-- ────────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────────
-- 5. auto_version_trigger CASE — add customers → customer_id
-- ────────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────────
-- 6. fn_audit_trigger CASE — add customers → customer_id
-- ────────────────────────────────────────────────────────────────────
