-- Translate exclusion_violation into the honest TIMESLOT_OCCUPIED error
-- so the voice agent can tell the caller "someone just booked that
-- time" instead of surfacing a raw SQL exception.
--
-- Pairs with 20260501000000_atomic_booking_exclusion_constraints.sql.
-- The constraints there are the actual race-fix; this migration wraps
-- the two RPCs' final INSERT in an exception handler so the LLM-facing
-- response stays in the {success, error_code} shape it already knows.
--
-- Both RPCs keep their pre-INSERT NOT EXISTS checks. Those still serve
-- a purpose — the diagnostic specific-error-code branches (e.g.
-- NO_SKILLED_EMPLOYEE, EMPLOYEE_NOT_SCHEDULED) only fire when the
-- pre-check fails, which is the non-race case. The exception handler
-- catches only the race itself: caller A's INSERT commits between
-- caller B's pre-check and caller B's INSERT.

-- ────────────────────────────────────────────────────────────────────
-- 1. book_appointment_atomic — wrap final INSERT
-- ────────────────────────────────────────────────────────────────────
...
BEGIN
    ...
    -- Final INSERT. The exclusion constraints
    -- (appointments_no_resource_overlap, appointments_no_employee_overlap)
    -- catch the race where another transaction inserted a conflicting
    -- appointment between our pre-check above and this INSERT. Translate
    -- the SQLSTATE 23P01 into the same "already booked" message the
    -- pre-check uses, so callers can't tell the difference.
    BEGIN
...
-- ────────────────────────────────────────────────────────────────────
-- 2. book_with_scheduling_atomic — wrap final INSERT
-- ────────────────────────────────────────────────────────────────────
...
BEGIN
    ...
    -- Final INSERT. The exclusion constraints catch the race where
    -- another transaction inserted a conflicting appointment between
    -- our find-pair scan above and this INSERT. We translate that into
    -- TIMESLOT_OCCUPIED — same code the pre-check returns — so the
    -- voice agent can honestly tell the caller "someone just booked
    -- that time, want to try another slot?"
    BEGIN
        INSERT INTO appointments (
            tenant_id, resource_id, customer_id, start_time, end_time,
