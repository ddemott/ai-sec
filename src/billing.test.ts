...
describe('Stripe Lite — Billing', () => {
  ...
  it("subscription_status defaults to 'inactive'", async () => {
    ...
    const res = await client.query(`SELECT subscription_status FROM tenants WHERE tenant_id = $1`, [
      tenantId,
    ]);
  ...
  it("subscription gate: 'active' status should allow access", async () => {
    ...
    await client.query(`UPDATE tenants SET subscription_status = 'active' WHERE tenant_id = $1`, [
      tenantId,
    ]);
    const res = await client.query(`SELECT subscription_status FROM tenants WHERE tenant_id = $1`, [
      tenantId,
  ...
  it("subscription gate: 'past_due' status should block access", async () => {
    ...
    await client.query(`UPDATE tenants SET subscription_status = 'past_due' WHERE tenant_id = $1`, [
      tenantId,
    ]);
    const res = await client.query(`SELECT subscription_status FROM tenants WHERE tenant_id = $1`, [
      tenantId,
  ...
  it("subscription gate: 'canceled' status should block access", async () => {
    ...
    await client.query(`UPDATE tenants SET subscription_status = 'canceled' WHERE tenant_id = $1`, [
      tenantId,
    ]);
    const res = await client.query(`SELECT subscription_status FROM tenants WHERE tenant_id = $1`, [
      tenantId,
  ...
  describe('Error diagnostics', () => {
    it('subscription gate logic correctly identifies blocked statuses', async () => {
      ...
      for (const status of allowedStatuses) {
        await client.query(`UPDATE tenants SET subscription_status = $1 WHERE tenant_id = $2`, [
          status,
          tenantId,
        ]);
        const res = await client.query(
