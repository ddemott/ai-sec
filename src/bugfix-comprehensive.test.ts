...
describe('Tenant isolation on DELETE/UPDATE queries', () => {
  ...
  it('DELETE customer with correct tenant succeeds', async () => {
    ...
    const check = await client.query('SELECT customer_id FROM customers WHERE customer_id = $1', [
      custA,
    ]);
  ...
  it('UPDATE customer with correct tenant succeeds', async () => {
    ...
    const res = await client.query(
      'UPDATE customers SET name = $1 WHERE customer_id = $2 AND tenant_id = $3 RETURNING customer_id',
...
describe('HubSpot webhook timestamp validation', () => {
  function validateTimestamp(raw: string): { valid: boolean; error?: string } {
    ...
    if (!Number.isFinite(timestampMs) || timestampMs <= 0) {
      return {
        valid: false,
        error: `Invalid timestamp header: "${raw}" (WHO: webhook caller, WHAT: timestamp parse, HOW: not a finite positive number)`,
      };
    }
    ...
    if (requestAge > 5 * 60 * 1000) {
      return {
        valid: false,
        error: `Timestamp too old: age=${requestAge}ms (WHO: webhook caller, WHAT: replay protection, WHEN: ${new Date(timestampMs).toISOString()}, HOW: exceeds 5min window)`,
      };
    }
    if (requestAge < -30_000) {
      return {
        valid: false,
        error: `Timestamp in future: age=${requestAge}ms (WHO: webhook caller, WHAT: clock skew, WHEN: ${new Date(timestampMs).toISOString()}, HOW: more than 30s ahead)`,
      };
    }
    return { valid: true };
  }
...
describe('Knowledge file upload validation', () => {
  ...
  function validateFile(filename: string): { valid: boolean; error?: string } {
    ...
    if (!allowedExtensions.includes(ext)) {
      return {
        valid: false,
        error: `Unsupported file type "${ext}" (WHO: uploader, WHAT: file type check, WHERE: /knowledge/ingest, HOW: allowed=${allowedExtensions.join(',')})`,
      };
    }
    return { valid: true };
  }
  ...
  function validateChunkCount(count: number): { valid: boolean; error?: string } {
    if (count > MAX_CHUNKS) {
      return {
        valid: false,
        error: `Too many chunks: ${count} (WHO: uploader, WHAT: chunk limit, WHERE: /knowledge/ingest, HOW: max=${MAX_CHUNKS}, split file into smaller parts)`,
      };
    }
    return { valid: true };
  }
