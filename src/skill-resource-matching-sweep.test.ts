...
import {
    ...
    createScheduleEntry, beginTestTransaction, rollbackTestTransaction
} from "./test-utils";
...
describe("Skill + resource matching — per-industry HAPPY paths", () => {
    it("automotive: books brake job with brakes-skilled tech in a brake-capable bay", async () => {
        ...
        await createServiceWithRequirements(root, tenantId, "Brake pad replacement", 60, ["brakes"], ["lift"]);
        ...
        expect(result.error_code, `expected success but got ${result.error_code}: ${result.error_message}`).toBeNull();
    ...
    it("mobile_tire: books tire mount with hyphenated 'tire-mount' skill on a truck", async () => {
        ...
        const techId = await createEmployee(root, tenantId, "Mike Rivera", ["tire-mount", "tire-balance"]);
        await createScheduleEntry(root, tenantId, techId, "2026-04-15", "08:00", "17:00");
        await createServiceWithRequirements(root, tenantId, "4-Tire Mount + Balance", 60, ["tire-mount"], ["mobile"]);
    ...
    it("auto_bays: books alignment requiring multi-skill match (alignment + lift capability)", async () => {
        ...
        const bay1 = await createResourceWithCapabilities(root, tenantId, "Bay 1", ["lift", "alignment-rack"]);
        await createResourceWithCapabilities(root, tenantId, "Bay 2", []);

        // Tech 1: alignment-skilled. Tech 2: oil-change only.
        const tech1 = await createEmployee(root, tenantId, "Tech Alpha", ["alignment", "tire-rotation"]);
        ...
        await createServiceWithRequirements(root, tenantId, "4-Wheel Alignment", 45, ["alignment"], ["alignment-rack"]);
...
describe("Skill + resource matching — error-code matrix", () => {
    ...
    it("TIMESLOT_OCCUPIED when the only candidate employee+resource pair is already booked", async () => {
        ...
        expect(second.error_code, `expected TIMESLOT_OCCUPIED, got ${second.error_code}`).toBe("TIMESLOT_OCCUPIED");
    });
...
describe("Skill + resource matching — cross-template guards", () => {
    it("does not route across tenants even when skill names collide", async () => {
        ...
        expect(result.success).toBe(false);
        expect(result.error_code, "must reject — no 'color' employee in auto tenant despite one existing in salon tenant").toBe("NO_SKILLED_EMPLOYEE");
