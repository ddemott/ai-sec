...
describe('Coverage Gap Detection', () => {
  ...
  it("should return 'partial' or 'gap' when service has limited coverage", async () => {
    ...
    const covered = brakes.filter(
      (r: CoverageRow) => r.status !== 'closed' && Number(r.coverage_pct) > 0
    );
  ...
  it('should include gap_hours array with uncovered hours', async () => {
    ...
    const alignmentMon = res.rows.find(
      (r: CoverageRow) => r.service_name === 'Alignment' && new Date(r.check_date).getDay() === 1
    );
    if (alignmentMon) {
  ...
  it('should return no rows when tenant has no services', async () => {
    ...
    const res = await client.query('SELECT * FROM check_coverage_gaps($1)', [tenantId]);
  ...
  it('should exclude soft-deleted employees', async () => {
    ...
    const empId = await seedEmployee('Deleted Worker', [], [1, 2, 3, 4, 5], '08:00', '17:00', [
      svcId,
    ]);

    await client.query(
      'UPDATE employees SET is_deleted = true, deleted_at = now() WHERE employee_id = $1',
      [empId]
    );
