 ...
 */
function requireOwner(req: AppRequest, reply: { status: (n: number) => { send: (b: unknown) => unknown } }): boolean {
  if (!req.auth) {
...
export function registerUserRoutes(
  ...
  app.get('/users', withHandler(async (req: AppRequest, reply) => {
  ...
  app.post('/users/invite', { config: { rateLimit: { max: 20, timeWindow: '1 hour' } } }, withHandler(async (req: AppRequest, reply) => {
    if (!requireOwner(req, reply)) return;
    ...
    try {
      ...
      if (err instanceof Error && (err as { code?: string }).code === '23505') {
        return reply.status(409).send({ success: false, error: 'A user with that email already exists for this business' });
      }
      throw err;
    }
    ...
    try {
      await sendUserInviteEmail(normalizedEmail, resetLink, INVITE_TTL_MINUTES, businessName, role);
    } catch (err) {
  ...
  app.patch('/users/:id/role', withHandler(async (req: AppRequest, reply) => {
    if (!requireOwner(req, reply)) return;
