...
async function seedComm(channel: 'email' | 'sms', recipient: string): Promise<void> {
  await setup.query(
    `INSERT INTO communications_history
       (tenant_id, channel, direction, recipient, subject, body, status)
     VALUES ($1, $2, 'outbound', $3, 'hi', 'body', 'sent')`,
...
describe('GET /communications/history → real DB', () => {
  it('HAPPY: type=all returns every row with total=5', async () => {
  ...
  it('HAPPY: the channel filter partitions rows (type=sms → only the 2 sms)', async () => {
  ...
  it('HAPPY: pagination — limit=2&offset=0 returns 2 rows but total still reflects the full set (COUNT OVER)', async () => {
    ...
    // total (5), not the page size (2) — the classic window-vs-limit trap.
