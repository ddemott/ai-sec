...
describe('ReminderProcessor metrics', () => {
    ...
    it('HAPPY: sms success → reminders_sent_total{channel=sms,outcome=success}+=1', async () => {
        ...
        const consents = {
            checkConsent: vi.fn().mockImplementation(
                async (_tenantId: string, _email?: string, _phone?: string, channel?: 'email' | 'sms') => channel === 'sms'
            ),
        } as unknown as ConsentService;
