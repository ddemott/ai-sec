...
export class SeventeenHatsAdapter extends BaseCRMAdapter {
  constructor(config: CRMConfig, tokenManager?: TokenManagerInterface) {
    super(config, tokenManager);
  }

  getProvider(): 'seventeenhats' {
    return 'seventeenhats';
  }
  ...
  async getCustomers(_since?: string): Promise<CRMCustomer[]> {
    throw new Error('17hats adapter not yet implemented');
  }
