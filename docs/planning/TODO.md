...
- [ ] **(code)** **Finish the dashboard component subdirectory migration** — **9** loose `.tsx` files at `dashboard/components/` (count 2026-09-04: ui tests, SetupWizard.test + types, phone/GoLivePanel.test). Main views sliced into appointments/, knowledge/, analytics/, scheduler/, voice/, shifts/. Remaining: move test files to parallel test structure or keep in ui/ if shared primitives.
