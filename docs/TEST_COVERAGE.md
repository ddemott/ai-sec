...
**Last refreshed:** 2026-05-13 — PK rename **pilot 28** closed the real-DB coverage gap from the May 12 sprint. New `src/pk-rename-coverage.test.ts` (626 lines, 30 tests) exercises every renamed PK column against actual Postgres: INSERT returns the column by name, SELECT/UPDATE/DELETE bind on it. Any regression to bare `id` now fails loudly instead of passing on mocks. Side sweep: 121 `id` → `<table>_id` edits across routes/services/tests/dashboard/e2e/seed.sql (pure renames). Backend 1,781 → 1,860 (+79); dashboard 620 → 631 (+11); agent 85 unchanged.

> Refresh history for 2026-05-09 through 2026-05-12 (PK rename sprint Parts 1+2, reminder-on-create wiring + UUID type fix, version-history-restore E2E, tenant-delete cascade FK fix, mobile-responsive E2E, scheduler real-time refresh, security pass 2, appointment cancel/restore) is in `RESOLVED.md`.
