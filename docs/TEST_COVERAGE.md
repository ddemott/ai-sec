...
**Headline counts last reconciled:** 2026-07-07 to the canonical living numbers in `CLAUDE.md` → Project Status (~2,328 backend + ~1,012 dashboard + ~496 agent, verified 2026-07-08 against real `test_db`/vitest). The **per-file V8 coverage %** + the e2e workflow tables below are from the **2026-05-22** run and are stale — re-run [Regenerating](#regenerating) before trusting a specific percentage.
...
|---|---|---|---|
| Backend (`npm test`) | ~2,328 | ✅ | ~190s |
| Dashboard (`cd dashboard && npm test`) | ~1,012 | ✅ | ~15s |
...
Total unit tests: ~3,836 passing (~2,328 backend + ~1,012 dashboard + ~496 agent). Counts are the 2026-07-08 CLAUDE.md figures; re-run for exact numbers.
