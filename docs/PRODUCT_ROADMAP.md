...
> and *prove* it is done with a command, query, or named test — no subjective judgment.
>
> **Roles:**
...
STATUS: 🟡 IN_PROGRESS (branch `fix/T-007-e2e-flakiness`; acceptance test passing locally, awaiting CI + merge)
OWNER: Claude-able
PRIORITY: HIGH
EFFORT: 4–6h
DEPENDS_ON: None
...
STEPS:
1. Replace timeout-based waits with `waitFor()` on actual DOM/API state. — done via the global `asyncUtilTimeout`.
2. For the E2E specs, wait on the `update-config` network response, then assert value in Postgres before reload. — done in `saveAiPersona()`; `voice-styles` gained the Postgres assertions it lacked.
3. Remove any machine-speed assertions. — none remained.
4. (Added) PIN the managed tenant before navigation so the tenant under test is chosen by the test, not by a list ordering; assert the config GET was for that tenant so a wrong-tenant session fails by name instead of as a value that "didn't persist" three steps later.
5. (Added) Scope every DB reset with `WHERE tenant_id = $1`. An unqualified reset is cross-spec data corruption the moment anyone raises `workers` above 1.
ACCEPTANCE_TEST:
```bash
