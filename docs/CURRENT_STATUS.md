...
| **Database** | Live | Supabase Postgres (managed), 104 migrations in repo, FORCE RLS on all tables. **15 migrations awaiting prod apply:** `20260501000000` (exclusion constraints) + `20260501000001` (RPC handlers); `20260505000000_user_roles.sql`; `20260511000000_employees_services_tenant_fk_cascade.sql` (orphan-row leak fix — see [[in-flight]]); the 14-migration PK rename sprint `20260512000000–13` (one ALTER TABLE per pilot plus three trigger rewrites). The PK renames are forward-only and have to land in pilot order on prod — any application code deployed against prod expects the renamed columns post-trigger-rewrite. |
