...
**Last updated:** 2026-05-15 (six-commit test-honesty + observability session. (1) `35a06e7` batch-4 any-types cleanup — new `AppFastifyInstance` alias across 46 files. (2) `f89e55a` **big find**: 524 DB-dependent tests across 50 files had been silent-passing whenever local DB was down. New `skipIfDbDown(ctx, () => dbAvailable)` helper + `REQUIRE_DB_TESTS=1` env var; sweep also fixed a latent fixture bug in `reminder-retry-worker.test.ts` (commit `d934e68`) that violated the `appointments_end_time_15min` 15-minute-boundary check. (3) `5101d76` wired `REQUIRE_DB_TESTS=1` into the backend CI job. (4) `057f12d` the new CI guard immediately caught a stale `localhost:5433/postgres` DATABASE_URL fallback in `architecture-review-fixes.test.ts`. (5) `fc4dedf` audit follow-up: 9 more silent-skip sites in `token-refresh.test.ts` + `jwt-logging.test.ts` (server-not-running pattern via fetch try/catch) migrated to `Fastify.inject()`; table-missing silent-skip stripped from `architecture-review-fixes` `withClient` helper. (6) `f8c3bc2` `@sentry/node` wired into backend + agent, gated by `SENTRY_DSN` (no-op when unset). All 6 commits CI-green; backend 1,903 → 1,910, agent 85 → 91, dashboard 631 unchanged. Previously 2026-05-13: PK rename **pilot 28**: real-DB integration coverage. New `src/pk-rename-coverage.test.ts` (626 lines, 30 tests) exercises every renamed PK column against actual Postgres — INSERT/SELECT/UPDATE/DELETE all bind on the new `<table>_id` column — closing the 56% gap the May 12 sprint left behind (mocked-only or untouched). Side sweep: 121 follow-up code-residue edits across backend routes/services/tests, shared scheduling types, dashboard components, e2e specs, and `supabase/seed.sql` — pure `id` → `<table>_id` renames, no behavior change. Backend 1,781 → 1,860; dashboard 620 → 631; agent 85 unchanged. Previously 2026-05-12: PK naming convention conversion **fully complete in two parts**. Part 1: pilots 1–16 covered every domain entity table (`record_versions`, `tenant_skills`, `reminder_schedules`, `consent_records`, `opt_out_records`, `voice_sessions`, `tenant_docs`, `tenant_integration_settings`, `users`, `services`, `resources`, `employees`, `employee_schedule`, `appointments`, `customers`, `tenants`). Part 2: pilots 17–25 closed the 9 non-domain leaf tables left out of Part 1 (`user_feedback`, `soft_reservations`, `audit_log`, `unanswered_questions`, `phone_verifications`, `password_resets`, `call_transcripts`, `call_summaries`, `entity_sync_map`). Two code-residue pilots followed: pilot 26 swept ~50 stale `WHERE id` / `RETURNING id` refs across `dashboard/e2e/` specs (Playwright suite, not run by unit CI), and pilot 27 aligned 8 test mocks plus fixed one production hot-path bug (`agentTools.ts /service-catalog` had bare `SELECT id FROM services` — hidden by a mock). 26 new migrations total (20260512000000–25). Trigger rewrites: `auto_version_trigger` + `fn_audit_trigger` PK-aware via TG_TABLE_NAME CASE; `create_default_resources` updated for `NEW.tenant_id`; `link_orphaned_transcripts` recreated for `call_transcript_id`. Backend 1,781 / dashboard 620 / agent 85 all green. Migration count 90 → 116. **Every single-column PK in the public schema now follows `<table_singular>_id`**; verification query returns zero rows. Only `tenant_calendar_settings` and `appointment_sync_map` remain (decision-pending — both have single-column PKs on FK columns, not surrogates).)
...
| **`DASHBOARD_URL` env var** | IN FLIGHT (user) | Outstanding 12+ days. Stripe checkout + OAuth redirects depend on it. | User sets it on Railway → ai-sec service → Variables (~2 min). |
| **`SENTRY_DSN` env var** | IN FLIGHT (user) | Code shipped 2026-05-15 (commit `f8c3bc2`); dormant until DSN is set. Backend + agent both gated by env var — no-op when unset. | User signs up at sentry.io, creates a Node.js project, sets `SENTRY_DSN` on both Railway services (~5 min). See `docs/DEPLOYMENT.md` → "Observability — Sentry error monitoring". |
...
### May 15: Test-skip honesty trilogy + Sentry wiring (backend 1,903 → 1,910, agent 85 → 91)

Six commits, all on `origin/main`, all CI-green:

- **`35a06e7` chore(types): batch-4 any-types cleanup (`AppFastifyInstance` alias).** Continuation of batches 1–3 (`e371374` / `ed82fd8` / `4fe2363`). New `src/types/fastify.ts` exports `AppFastifyInstance = FastifyInstance<any, any, any>` as the single source of truth for the loose Fastify generic that route modules + middleware hooks receive. Replaced 46 inline declarations + the two `as any` casts in `src/index.ts` at the `registerJwtAuthHook(app)` / `tenantMiddleware(app)` call sites. One `as any` remains on the Fastify constructor options where http/https ternary widens past the union — annotated with eslint-disable + rationale.

- **`f89e55a` test: skipIfDbDown helper — silent-skip on DB-down → SKIPPED, not PASSED.** **The big find of the day.** Every test file using the `dbAvailable` pattern caught the `ECONNREFUSED` in `beforeAll`, set `dbAvailable=false`, and let each `it` body return early via `if (!dbAvailable) return;`. Vitest reported those tests as PASSED — 524 DB-dependent tests across 50 files had been silent-passing whenever the local DB wasn't up. Surfaced from one log line in a routine test report: `[booking-concurrency] DB not available: Error: connect ECONNREFUSED 127.0.0.1:5433`. New `skipIfDbDown(ctx, () => dbAvailable)` helper in `src/test-utils.ts`; each file gets `beforeEach((ctx) => skipIfDbDown(ctx, () => dbAvailable));`. `REQUIRE_DB_TESTS=1` env var turns DB-down into a hard `beforeEach` failure for CI. Sweep performed via `/tmp/sweep-skip.mjs` (mechanical regex transform of 45 files; 4 holdouts manual). Three behaviors verified: DB up → 1,903 / 0 skipped; DB down → 1,379 / 524 skipped (formerly silent-passing); DB down + `REQUIRE_DB_TESTS=1` → hard failure. Also fixed a latent fixture bug surfaced by the sweep: `reminder-retry-worker.test.ts` (commit `d934e68`, 2026-05-14) had never actually run against a real DB — its INSERT used `NOW() + INTERVAL '1 day 30 min'` for `end_time`, violating the `appointments_end_time_15min` check constraint. Fixed with `date_trunc('hour', NOW())` base.

- **`5101d76` ci: REQUIRE_DB_TESTS=1 on backend job.** Wires the new env var into `.github/workflows/ci.yml`'s "Run backend tests" step + extends the Postgres-service-container comment block to reference both layers of defense (the service ensures DB is provisioned; the env var refuses to silent-skip if it's somehow unreachable).

- **`057f12d` fix(test): align architecture-review-fixes fallback to test_db.** The first CI run for `5101d76` failed exactly as the new guard was designed to. `architecture-review-fixes.test.ts:29` had a stale `localhost:5433/postgres` DATABASE_URL fallback while every other test uses `test_db`. CI's Postgres service container only creates `test_db`, so connection failed → `dbAvailable=false` → guard threw with the explicit error message. **The new guard caught a pre-existing inconsistency that had been hidden behind the silent-skip pattern.**

- **`fc4dedf` test: kill remaining silent-skip shapes the sweep missed.** Audit follow-up after the user asked "next task" post-CI-green. Found 3 more silent-skip shapes the `dbAvailable` sweep didn't cover. (1) `token-refresh.test.ts` (6 sites) + `jwt-logging.test.ts` (3 sites) had a local `apiFetch` helper catching fetch errors → returning null → every `it` doing `if (!res) return;`. CI never starts a Fastify server, so all 13 tests across both files had been silent-passing. Migrated both to `Fastify.inject()` against an in-memory app with the real JWT auth hook + auth routes. (2) `architecture-review-fixes.test.ts` `withClient` helper caught `does not exist` / `relation` errors and silently returned — removed the catch so schema-drift regressions surface. Surfaced two subsidiary bugs during the inject() migration: JWT_SECRET hoisting (ES module imports run BEFORE statement-level env assignment — fixed with `vi.hoisted()`) and a UUID type mismatch in test payloads (`user_id: 'test-user'` threw `invalid input syntax for uuid` inside the middleware's `password_changed_at` lookup → 500 — switched to a real UUID literal).

- **`f8c3bc2` feat(observability): wire Sentry error monitoring on backend + agent.** `@sentry/node` on both Node services with the same gating shape as `BETTER_STACK_TOKEN`: no-op when `SENTRY_DSN` unset, so local dev / `npm test` make zero network calls. Backend (`src/services/sentry.ts`): `initSentry()` at top of `src/index.ts` before any other import; `logError()` in `src/middleware.ts` now calls `captureException()` after Pino + errorsTotal — so every error routed through `withHandler` flows to Sentry with `tenant_id` + `route` + `event` tagged. Fastify `setErrorHandler` also captures. Agent (`agent/src/sentry.ts`): same shape; captures fallback-triggered events at both `runFallback()` callsites. Sentry's default integrations cover `uncaughtException` + `unhandledRejection`. Scope: errors only (`tracesSampleRate: 0`, `profilesSampleRate: 0` — the metrics module already covers latency/throughput). Single DSN hosts both services; the `service` tag (`ai-sec-backend` vs `ai-sec-agent`) splits them in the Sentry UI. 13 new tests (7 backend + 6 agent) pin env-gating, init idempotency, capture context. Dashboard Sentry deferred. **IN FLIGHT (user)** — dormant until `SENTRY_DSN` is set on Railway services.

After-state: backend 1,903 → 1,910 (+7), agent 85 → 91 (+6), dashboard 631 unchanged. Zero TS errors across backend / agent / dashboard. All 6 commits CI-green.
...
### Test Count (verified 2026-05-15 against real Postgres + dashboard)
- **1,910 backend tests + 631 dashboard tests = 2,541 passing**, 0 failures, 0 skips
- 85 agent tests (`cd agent && npm test`)
- 85 Playwright e2e + 7 skip-guarded (run with `SYNC_TEST_RECORDER=1` to flip them on)
...
| **Database** | Live | Supabase Postgres (managed), 120 migrations in repo, FORCE RLS on all tables. **34 migrations awaiting prod apply:** `20260501000000` (exclusion constraints) + `20260501000001` (RPC handlers); `20260505000000_user_roles.sql`; `20260511000000_employees_services_tenant_fk_cascade.sql` (orphan-row leak fix — see [[in-flight]]); the **26-migration PK rename sprint `20260512000000–25` — complete** (every single-column PK now uses `<table_singular>_id`; auto_version_trigger + fn_audit_trigger CASE TG_TABLE_NAME-aware; `create_default_resources` and `link_orphaned_transcripts` recreated); `20260513000000_service_employee_tenant_fk_cascade.sql`; `20260513000001_tenants_notification_preferences.sql`; `20260513000002_opt_out_records_fk_rename.sql` (pilot 4 deferred FK rename closed); `20260513000003_deleted_customers_view_recreate.sql` (last residual `AS id` alias on a renamed table). The PK renames are forward-only and have to land in pilot order on prod — any application code deployed against prod expects the renamed columns post-trigger-rewrite. Apply via `npm run db:migrate -- "postgres://<prod-url>"`; the runner reads `schema_migrations` per-DB and only applies what's missing, so re-running is safe. |
...
| **Tests** | 1,910 backend + 631 dashboard + 91 agent = 2,632 passing + 88 QA assertions | All green (verified 2026-05-15 against real DB + dashboard, CI green on commit `f8c3bc2`), 0 skips, zero TS errors |
| **Playwright e2e** | 85 passed / 7 skipped | Against live dashboard |
