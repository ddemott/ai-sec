...
- [ ] **🐛 BUG (infra) — `supabase/baseline.sql` is STALE; local E2E uses a schema missing 3 shipped tables.** Found 2026-06-19. `rebuild-db.sh` prefers the single-file `baseline.sql` (2026-05-18 squash) then `setup-db.sh --baseline` marks EVERY later migration applied **without running it**. So tables added after the squash — `knowledge_suggestion` (20260612), `customer_messages` (20260616), `ai_cost_events` (20260618) — never get created in any DB built via the baseline path (local Playwright `globalSetup`, `npm run db:rebuild`). CI dodges this: the e2e job builds test_db from the **full migration chain** (`setup-db.sh` + `seed-db.sh`) and sets `PLAYWRIGHT_SKIP_DB_RESET=1` so `globalSetup` doesn't re-run the baseline (see `.github/workflows/ci.yml` comment ~L249-255). **Impact:** any E2E touching those 3 tables fails locally with `relation "..." does not exist`; the local KnowledgeSuggestions / messages / cost surfaces break after a `db:rebuild`. **Fix options (own PR — infra, not feature):** (a) regenerate `baseline.sql` from the chain (pg_dump 16.14 matches the header → low-diff) and add it to a regen script + the schema-alignment guard; or (b) make baseline-mode still RUN post-squash migrations instead of blanket-marking them applied. Workaround until fixed: rebuild local DB via the chain — `psql "$DATABASE_URL" -c 'DROP SCHEMA public CASCADE; CREATE SCHEMA public;'` then `bash scripts/setup-db.sh && bash scripts/seed-db.sh`, and run Playwright with `PLAYWRIGHT_SKIP_DB_RESET=1`. The `verify-schema-alignment` guard should have caught this — check why it didn't.
...
- [~] **E2E + simulate coverage for the step.** PARTIAL 2026-06-19 (`feat/question-bank-shared`). Added the CI-safe half to `dashboard/e2e/knowledge-base.spec.ts` (tests 11-13): seed a `knowledge_suggestion` → GET review queue → PATCH confirmed ingests into the live KB (source=`website-scan`, status flips) / PATCH rejected discards / cross-tenant approve → 404. Run-verified green (14/14) against a migration-chain DB. Also added a backend handler regression test (`src/routes/knowledge.importWebsite.test.ts`, mocked fetch) asserting a NON-EMPTY question list reaches OpenAI. **Still TODO:** the OpenAI website-scan happy-path itself (real LLM + external fetch — not CI-safe; needs a mock-OpenAI hook or a live smoke), wizard UI click-path E2E (URL paste → suggestions render → approve), and `simulate.sh tools` import coverage. Gate on the RAG accuracy eval.
