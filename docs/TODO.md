...
- **Voice / Telnyx**: **SIP chain FIXED + verified 2026-06-09** (real 19s inbound call, Q.850 cause 16, agent joined room). Root cause was a stale FQDN in the Telnyx SIP connection (pointed at the deleted LiveKit project `ai-secretary-nmlkkmgf`); repointed → `3jay24s076x.sip.livekit.cloud:5060`. **Current blocker:** agent tenant resolution — `buildSessionContext` returns null (no `tenant_id` in dispatch/room metadata) → agent plays "system issue" fallback. Fix = Option A (hardcode dispatch metadata, smoke test) then Option B (DID→tenant lookup in `agent/src/sessionContext.ts`). **Source of truth: `docs/TELNYX_HANDOVER.md`** (prior PSTN/recycled-DID docs are superseded).
...
- [~] **Telnyx provisioning — DONE 2026-06-02.** Account for Thinking Hammer LLC funded ($10) + upgraded (trial 1-order cap lifted). SIP Connection `livekit-outbound` (`2945038451784812111`) → FQDN `ai-secretary-nmlkkmgf.sip.livekit.cloud:5060`. `TELNYX_API_KEY` + `TELNYX_SIP_CONNECTION_ID` set (local `.env`; verify on Railway `ai-sec`). Number **`+1 630-866-1960`** purchased (id `2973794140900296302`), routed to `livekit-outbound`, connection activated. Old `+1-630-937-9478` is dead (order deleted). **2026-06-09 UPDATE (supersedes 2026-06-04 PSTN theory):** SIP transport FIXED — the real cause was a stale FQDN in the Telnyx connection pointing at the deleted LiveKit project subdomain; repointed → `3jay24s076x.sip.livekit.cloud:5060`. Inbound now reaches LiveKit and the agent joins the room (verified by a real 19s call). **Remaining blocker = agent tenant resolution** (no `tenant_id` in dispatch metadata → fallback message). See **`docs/TELNYX_HANDOVER.md`** for Option A/B. The trunk now carries both `+16308661960` and `+16308229086`.
