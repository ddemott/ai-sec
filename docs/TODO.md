...
- [ ] **RAG: "address" queries don't retrieve the address doc (pre-existing, found 2026-06-23).** `./scripts/simulate.sh rag` shows "what's your address" / "could you tell me your address?" fall back to "I don't have info" instead of returning the seeded address KB doc. **Confirmed pre-existing, NOT a regression** from #39's threshold change — fails at both the old 0.5 and new 0.30 similarity thresholds (pre-#39 eval also missed it; verified by re-running the eval against `65713f1`). Root cause is a vocabulary gap, not threshold: the seeded doc is a Q/A pair (`"Where are you located?"` / `"We are located at 123 Main Street in downtown."`) which shares no words with the caller phrasing "what's your **address**" — so the cosine similarity stays below even the new 0.30 floor under `text-embedding-3-small`. Notably this is a known case (`sim-rag.mjs` labels it the "regression guard for the 2026-06-12 address-gap fix"), and #39's query expansion was specifically meant to bridge vocabulary gaps like this — yet "address"→"located" still isn't expanded enough to clear retrieval. Options to investigate: (a) teach `shared/expandQueryForEmbedding.ts` the address↔located / hours↔open / phone↔call synonym intents (the expansion is the intended fix surface); (b) ingest location/hours facts with a synonym-rich title/alias so the embedding overlaps more phrasings; (c) a structured fast-path for address/hours/phone that bypasses RAG. Low-urgency quality item; surfaced by the now-stricter 9-question eval.
