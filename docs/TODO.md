...
- [ ] **Booking RPCs ignore `is_deleted` in overlap/availability checks** (surfaced by Copilot on PR #137, verified **pre-existing** — the buffer migration only added `± v_buffer` padding; the `status = 'scheduled'` + absent-`is_deleted` handling is byte-identical to `20260512000016` already on prod). Across `book_appointment_atomic`, `book_with_scheduling_atomic`, `check_availability_with_tz` the overlap / `NOT EXISTS` availability / "already booked" diagnostic subqueries filter only `status = 'scheduled'`, not `is_deleted`. A soft-deleted appointment that still has `status='scheduled'` (is_deleted=true) would wrongly block a booking / hide an opening. Not buffer-specific → a separate correctness pass adding `AND (a.is_deleted IS NULL OR a.is_deleted = false)` to those appointment subqueries in one migration (verify no path legitimately relies on counting soft-deleted rows first). Low urgency unless the app leaves `status='scheduled'` on soft-delete.
