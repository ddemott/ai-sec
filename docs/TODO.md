...
- [ ] **UI — hero feature grid looks incomplete (empty 4th slot)** (reported 2026-07-01, Dale, w/ screenshot). The hero `feat-grid-hero` (`dashboard/app/page.tsx`) shows **3** outcome cards ("Nobody Goes to Voicemail", "Remembers Every Customer", "Handles It All in One Call") in a 2-col layout → the bottom-right cell is empty and reads unfinished. **Fix:** add a **4th** card in the same punchy outcome-story style. Recommended (fills a gap the other 3 don't + is the differentiator): **"Shows You Why You're Losing Jobs"** — "It logs why every caller didn't book — no Saturday slot, wrong service, sticker shock — so you finally see the pattern." Alternatives: **"Confirms Before They Hang Up"** (instant SMS proof) or **"Answers Anything About Your Shop"** (RAG from your own docs). Match the existing card markup (`feat-icon` + `feat-h` + `feat-p`).
- [ ] **🐛 BUG — deleting a customer orphans their appointments (can't cancel)** (reported 2026-07-01, Dale — hit while clearing test data, e.g. "Ab Smith"). Customer delete is now a **soft-delete** (`is_deleted=true`, PR #146); their appointments stay, but the schedule/list queries join customers `WHERE is_deleted=false`, so a deleted customer's appointment **vanishes from view** — an invisible booking can't be clicked "Cancel", and it may still hold the slot. The backend `/appointments/:id/cancel` itself is fine (`UPDATE ... SET status='canceled'`, no customer join). **Fix (agreed direction):** when a customer is soft-deleted, in the same transaction **auto-cancel their UPCOMING appointments** (`status='canceled'`, frees the slot) and **keep past/completed** for history/analytics. Plus a one-time cleanup for already-orphaned appointments (surface soft-deleted customers' appointments as cancelable, or backfill-cancel upcoming appts of already-deleted customers — e.g. Ab Smith). Do NOT hard-delete appointments (loses history).
- [ ] **🐛 BUG — landing page: pricing toggle + mobile hamburger menu are dead** (found 2026-07-01 while building the capability grid). The landing renders `LANDING_HTML` via `dangerouslySetInnerHTML`, and **`<script>` set that way never executes** — so `window.setBilling` is `undefined` (clicking Annual doesn't change prices; verified in-browser) and the hamburger `addEventListener` never attaches (mobile menu broken). Only the `.reveal` animation works because it was moved to a React `useEffect`. **Fix:** move the pricing-toggle + hamburger logic into `useEffect`(s) in `LandingPage` (`dashboard/app/page.tsx`), same pattern the capability-grid toggle now uses (PR #151); remove the dead inline `onclick`/`<script>`.
