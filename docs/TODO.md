...
- [ ] **(code)** **Finish the dashboard component subdirectory migration** — **53** loose `.tsx` files at `dashboard/components/` (count 2026-08-29: was 57 after records slice). Eighth slice: CRMView, CRMIntegrationCard → `components/crm/`.
...
- [ ] Continue count-drift passes (route modules / migrations / test numbers) after any new route or migration; keep secondary docs synced. **2026-08-29 pass:** dashboard loose `.tsx` at `components/` is **53** (crm slice); `supabase/migrations/` still **189**.
