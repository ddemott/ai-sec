...
- [x] **`scheduleForAppointment` reminder double-seed** — FOUND + FIXED same day (2026-07-01, by `scheduleForAppointment.realdb.test.ts`). Idempotency guard added: the seed probes for an existing `scheduled` bundle and skips, so retries can't double-remind. Reschedule still reseeds (cancel-then-seed unaffected). Test flipped from behavior-pin to idempotency assertion.
