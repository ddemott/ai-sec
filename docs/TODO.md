...
- [ ] **(code)** **Finish the dashboard component subdirectory migration** — **81** loose `.tsx` files at `dashboard/components/` (count 2026-08-28: was 89, then 49). First slice: VersionBadge, ErrorBoundary, DemoBanner, SetupProgressPill → `components/ui/`.
...
- [ ] Continue count-drift passes (route modules / migrations / test numbers) after any new route or migration; keep secondary docs synced. **2026-08-28 pass:** dashboard loose `.tsx` at `components/` is **81** (ui primitives moved); `supabase/migrations/` still **189**.
