...
- [~] **(code)** **Extract `src/routes/knowledge.ts` into services** — **PARTIAL 2026-08-21** (PR #367). **1,092 → 847 lines**; `siteScrape.ts` (fetch + prose extraction + LLM Q&A), `ingestChunks.ts` (normalize → embed → cost → insert), `suggestionReview.ts` (transactional approve / reject) under `src/services/knowledge/`. **Tests came first, as their own commit:** coverage **74.77% → 83.18% lines**, adding the untested CRUD surface — two of those are SECURITY assertions (a list that lost its tenant filter shows one business another's policies; a `DELETE` keyed on id alone lets any owner destroy another tenant's entries by guessing a UUID) — plus the **AI-cost path**, which had no coverage at all because every existing case runs with `KNOWLEDGE_IMPORT_E2E_STUB=1`. That ledger silently reported 2.8–4.3% of the real bill on 2026-08-13, so the new test pins the exact arithmetic rather than "a number was written". Shape quirks pinned not harmonized: `GET /knowledge` returns a bare array, `GET /knowledge/unanswered` a `{success, questions}` envelope. **Still inline:** `/knowledge/add`, `/knowledge/import-website`, `/knowledge/import-document`, `/knowledge/explain`, `/knowledge/unanswered/:id/resolve`.
