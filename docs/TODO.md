...
- [ ] **Clean up `any` types in backend test files — 112 remaining (down from 215).** Earlier sweeps cleared 86 across `33f83cd` + `01b7009` (2026-05-04). 2026-05-05 cleared 17 in `src/middleware-helpers.test.ts` (the single largest offender) using `as unknown as AppRequest` / `as unknown as FastifyReply` for partial-mock structural casts and concrete shapes (`{ error: string }`, `{ name: string }`) for callback parameters. Remaining 112 across ~29 files; top of the queue: normalizer (12), bugfix-comprehensive (11), routeHelpers (10), provisioning (10), expandWeeklyToSchedule (8), coverage (7), auth (7). Pattern reference: `vi.mocked(mockFn)` for typed mock access, `as unknown as Type` for partial-mock structural casts, proper `Fastify`/`Pool` type imports for production-shape mocks.
