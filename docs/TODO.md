...
- [ ] **(code)** **Website scan fetches owner-supplied URLs with no scheme/host guard** (new, 2026-08-21). `src/services/knowledge/siteScrape.ts` `fetchAndExtractSiteText` does `fetch(url)` on whatever the owner typed. Copilot flagged this on PR #367; the fetch is pre-existing (moved verbatim from `src/routes/knowledge.ts`) so it was not mixed into the extraction. Hole is real: no http/https allowlist, no block on localhost / private / link-local (`169.254.169.254`). An authenticated owner can scan internal IPs via `POST /knowledge/import-website`. Fix: scheme allowlist + reject loopback/private/link-local literals (and preferably the resolved address, not just the hostname) before `fetch`. Tests first.
