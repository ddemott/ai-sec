...
- [ ] **Data portability & retention** — [x] **export DONE 2026-06-22**: `GET /export/tenant-data` (`src/routes/exportData.ts`) — owner-gated (front-desk 403, super-admin bypass), tenant-scoped JSON dump of 25 business/ops/compliance tables; `users` column-restricted (no password_hash); excludes `tenant_integration_settings`/security-token tables; `record_counts` + attachment header; 4 route tests. _Chose JSON over ZIP-of-CSV to avoid a new archiver/csv dep (no-deps-on-spec); packaging layer deferred until a customer asks._ **Remaining**: GDPR/CCPA hard-purge (anonymize `voice_sessions` phone+transcript, delete notes/preferences, audit-write); automated retention/purge worker; owner-facing audit-log view from `audit_log`.
- [ ] **Website scan polish** — [x] **rate-limit DONE 2026-06-22**: per-tenant token-bucket guardrail on `POST /knowledge/import-website` (`src/services/scanRateLimit.ts`, default 3-scan burst + 3/hour refill, env-tunable) → 429 when dry; skipped in the E2E stub path; 6 unit tests + a route 429 test. **Remaining**: wizard UI click-path E2E + `simulate tools` import coverage (API/unit layers done); periodic re-scan scheduler for stale KB.
...
- [ ] **Docs / runbooks** — [x] **incident + telephony runbook DONE 2026-06-22**: `docs/RUNBOOK.md` — triage signals + playbooks for agent-silent, reminders-not-sending, Stripe-webhook-400, backend-down, DB-pool-saturation, and the full Telnyx→LiveKit→agent telephony troubleshooting path (incl. REFER/transfer + blocked-caller-ID OTP). **Remaining**: owner-facing admin guides ("how to read analytics", FAQ) in the dashboard/help surface.
