...
- [ ] **Audit Front Desk view for non-technical operators — IN PROGRESS.** Audit shipped 2026-05-07 (`docs/sessions/2026-05-07-front-desk-audit.md`); walked every primary task (book a call-in, look up tomorrow's schedule, mark someone unavailable, find a customer) and produced a six-item priority punch list (3 P0 launch-blockers, 3 P1/P2 polish). Punch list status: **all 3 P0 items + P1 #4 done 2026-05-07** — #1 Quick Book hoist (`1b60b48`), #2 Mark off today on `StaffProfileCard`, #3 searchable customer combobox via shared `CustomerCombobox` extraction, #4 empty-cell click → Quick Book prefilled (Staff sub-tab keyboard-accessible + Calendar sub-tab `selectable+onSelectSlot`). P1 #5 (default Schedule tab to staff — needs design call) and P2 #6 (Yesterday/Today/Tomorrow chips) still open. Track punch-list close-out in the audit doc, not here.
...
- [ ] **Remove unused Jest references from the project.** Audit confirmed 2026-05-07 that the entire test stack runs on Vitest 4.0.18 (`vitest.config.ts` × 3 workspaces, `npm test` → `vitest run` in all three). Zero `jest.fn` / `jest.mock` / `jest.spyOn` calls anywhere in `src/` / `dashboard/` / `agent/`; zero `from "jest"` or `from "@jest/..."` imports. Yet root `package.json` still declares `"jest": "^30.2.0"` in devDependencies — pure dead weight. Action: drop the root `jest` devDependency, sweep for any other stale references (lingering `@types/jest`, leftover jest config files, README mentions), `npm install` to refresh `package-lock.json`. **Keep `@testing-library/jest-dom`** — it's just the matcher library and works natively with Vitest via `dashboard/tsconfig.json`'s `"types": ["vitest/globals", "@testing-library/jest-dom/vitest"]`.
