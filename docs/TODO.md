...
- [ ] **(code)** **Finish the dashboard component subdirectory migration** — **68** loose `.tsx` files at `dashboard/components/` (count 2026-08-28: was 71 after layout slice). Fourth slice: CommsSentView, ReminderDeliveryStats → `components/communications/`.
...
- [ ] Continue count-drift passes (route modules / migrations / test numbers) after any new route or migration; keep secondary docs synced. **2026-08-28 pass:** dashboard loose `.tsx` at `components/` is **68** (communications slice); `supabase/migrations/` still **189**.
