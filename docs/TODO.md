...
- [ ] **(code)** **`estimateCost` silently bills $0 for any model missing from `PRICING`** (new, 2026-08-21). `src/services/aiCost.ts` does `PRICING[params.model] || { input: 0, output: 0 }`, so a model added at a call site but not to the table costs nothing and the ledger reports that as fact. This is **the exact shape of the 2026-08-13 under-count** the file's own header comment describes — `gpt-4.1-mini` fell through this branch and 77–83% of every call's cost recorded as $0.00 while the real bill was ~$0.06/call. The comment was added; the fallback was not. Every current model IS in the table, so nothing is wrong today — the defect is that the next one added will be wrong silently. Make the miss loud: bump `errors_total{event="ai_cost_unpriced_model"}` and log the model name, rather than returning a confident zero. Found while consolidating three duplicate price constants out of `services/knowledge` (PR #367); deliberately left out of that commit as a different subsystem.
