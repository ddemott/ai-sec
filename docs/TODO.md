...
- [ ] **Within-file dedup pass on `improvement-ideas.md`.** The automated journal is producing duplicate task suggestions: e.g. "Replace resource route validation branches with sendValidationError" appears at lines 112, 272, AND 539 of the same file (3 copies, same date 2026-04-20). The 2026-04-24 journal batch also re-proposes "Extract tenant provisioning preflight checks into a route-local helper in provisioning.ts" which is already at line 1442 of HEAD. Sweep obvious within-file repeats to one canonical entry. Separate concern: the journal generator itself is broken — fix that or it'll re-duplicate after each cleanup pass.
