...
- [ ] **(code)** **Finish the dashboard component subdirectory migration** — **75** loose `.tsx` files at `dashboard/components/` (count 2026-08-28: was 81 after ui slice). Second slice: LoginView, ProfileView, FirstRunTour → `components/auth/`.
...
- [ ] Continue count-drift passes (route modules / migrations / test numbers) after any new route or migration; keep secondary docs synced. **2026-08-28 pass:** dashboard loose `.tsx` at `components/` is **75** (auth slice); `supabase/migrations/` still **189**.
