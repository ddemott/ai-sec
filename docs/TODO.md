...
- [ ] **Audit Front Desk view for non-technical operators — IN PROGRESS.** Audit shipped 2026-05-07 (`docs/sessions/2026-05-07-front-desk-audit.md`); walked every primary task (book a call-in, look up tomorrow's schedule, mark someone unavailable, find a customer) and produced a six-item priority punch list (3 P0 launch-blockers, 3 P1/P2 polish). Punch list status: **all 3 P0 items + P1 #4 + P1 #5 done 2026-05-07** — #1 Quick Book hoist (`1b60b48`), #2 Mark off today on `StaffProfileCard`, #3 searchable customer combobox, #4 empty-cell click → Quick Book prefilled, #5 default Schedule sub-tab flipped to Staff (with narrative copy reworked to remove the "Calendar is the recommended default" framing). Only P2 #6 (Yesterday/Today/Tomorrow chips on `SchedulerDateNav`) still open. Track punch-list close-out in the audit doc, not here.
