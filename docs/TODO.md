...
- [ ] **Product expansion** — booking widget/embed; granular RBAC beyond owner/front_desk; white-label / reseller theming; public API; ~~CSV export (calls/appointments/customers)~~ **DONE 2026-07-04 (#189)** — PDF + analytics-export still open; SSO/SAML; international numbers (US-centric today); multi-DID per tenant.
...
- [ ] **[REVIEW]** `VoiceCallsView` — list layout, outcome-badge legibility, transcript/summary display, filter UX, meaning of "abandoned".
  ...
  - _Partial 2 (2026-07-03, branch `fix/ux-review-voicecalls-a11y`):_ keyboard a11y on the call/message lists — the active-call, history-call, and message rows were all click-only `<div>`s (unreachable by keyboard, announced as plain text). Each is now a `role="button"` + `tabIndex` + Enter/Space handler + accessible name + `aria-pressed` (selected state) + focus ring. +1 a11y test in `VoiceCallsView.test.tsx` (39 total green); dashboard tsc clean.
...
- [ ] **[REVIEW]** `EmployeeManagementView` — add-employee form, per-card skill assignment, deactivated-employee surfacing.
  - _Partial (2026-07-03, branch `fix/ux-review-employee-management`):_ fixed a silent-failure defect + a11y gaps. Create/update failures were swallowed (`apiMutate` returns `{success:false}` not a throw, and the handlers had no `else`), so a rejected save — notably the new duplicate-name 409 — gave the owner zero feedback; both now `showToast` the backend error. A11y: add-form first/last inputs got `aria-label`s (were placeholder-only), the service-assignment toggles expose `aria-pressed`, and the Active switch got an accessible name. 3 component tests (`EmployeeManagementView.test.tsx`). **Still open (owner judgment):** per-card skill-assignment interaction model, and how deactivated staff surface (currently an "On Leave" badge — semantics vs the "Active" toggle wording is an owner call).
- [ ] **[REVIEW]** `ShiftManagementView` — team-size-conditional paths, copy-week discoverability.
  - _Partial (2026-07-03, branch `fix/ux-review-shift-management`):_ error handling was already solid (5/6 catches toast); the gap was keyboard a11y in the week grid. The shift block was a click-only `<div>` (no keyboard access) — now `role="button"` + `tabIndex` + Enter/Space handler + `aria-label` + focus ring; the icon-only delete-shift button got an `aria-label`. Verified via dashboard `tsc` + `eslint` (next/core-web-vitals a11y rules) clean — no render test added (the grid is date-window + ResizeObserver dependent, so a full-render assertion would be flaky). **Still open (owner judgment):** team-size-conditional paths (weekly-grid vs per-day), copy-week discoverability.
...
- [ ] **[REVIEW]** `SkillMatrixView` + `SkillAssignmentsView` + `SkillRelationshipMap` — grid legibility at scale, map value vs confusion, both-views-necessary.
  - _Partial (2026-07-03, branch `fix/ux-review-skill-views`):_ big a11y win on the matrix, which was unusable with a screen reader — every cell was an icon-only Check/X button with no accessible name. Each cell now announces `Add/Remove <service> for <entity>` + `aria-pressed`; the filter tabs (All/People/Places) expose `aria-pressed`; the search box got an `aria-label`. `SkillRelationshipMap` cancel-connecting button + `SkillMapNode` connect button got `aria-label`s (their node root was already keyboard-operable). `SkillAssignmentsView` was already clean (tab toggle has `aria-pressed`). 2 new tests (`SkillMatrixView.test.tsx`); existing skill tests (45) still green, dashboard tsc clean. **Still open (owner judgment):** grid legibility at scale, whether the map view earns its keep, both-views-necessary — and note Cluster A still owns the map de-grade work (blocked on the scheduling-color review).
- [ ] **[REVIEW]** `DeletedRecordsPanel` + `RecordHistoryModal` — discoverability, restore flow, version-history comprehensibility.
  - _Partial (2026-07-03, branch `fix/ux-review-deleted-records-history`):_ fixed the a11y defects on both surfaces — icon-only controls (expand/collapse, view-history, copy-fields, both dialog close buttons) now carry accessible names; the deleted-records search input got a real `aria-label` (was placeholder-only); expand toggles expose `aria-expanded`; and both overlays are now proper `role="dialog"` + `aria-modal` + `aria-labelledby` with Escape-to-close, Tab focus-trap, and body-scroll lock via the shared `useFocusTrap` hook (matching Cluster C). 6 component tests (`DeletedRecordsPanel.test.tsx` + `RecordHistoryModal.test.tsx`). **Still open (owner judgment):** discoverability of the panel itself, the restore/copy-fields flow clarity, version-history comprehensibility, and the copy-target dropdown being empty for non-customer tables (feature is effectively customers-only today).
- [ ] **[REVIEW]** `/register` — form flow, field order, error handling, post-signup first experience.
  - _Partial (2026-07-03, branch `fix/ux-review-register`):_ the page was already a11y-strong (associated labels, autoComplete, `aria-hidden` icons, `role="alert"` errors, password-toggle `aria`, `minLength`, and existing 4-test coverage of the happy/409/network paths). Two small additions from the review: the password rule is now announced via `aria-describedby` → a visible "Use at least 6 characters" hint (was placeholder-only, which AT doesn't reliably read), and the `appearance-none` business-type `<select>` got a visible dropdown chevron + right padding so it reads as a select. +1 a11y test (5 total), dashboard tsc clean. **Still open (owner judgment):** field order, post-signup first-run experience.
...
- [ ] **Offers alternatives when the caller named a specific time** — verify + confirm THAT; only offer options if it's taken. (Partially served by the `time_changed` path above — when a specific pick isn't open the agent now names the booked slot + asks if they'd prefer another; the pre-booking "offer 2-3 first" behavior is still prompt-guided + live-unverified.)
