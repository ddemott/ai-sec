...
- [x] ~~**Extend the schema-alignment guard to catch RLS-flag drift** (the guard only diffed tables/columns, never noticed a table shipped with RLS enabled and zero policies — the message_delivery_status bug could recur silently).~~ DONE 2026-08-03 (**#316**, merged + deployed — prod `/ready` re-verified post-deploy: `rls_enforced=true`, `db_role=app_user`). `checkRlsPolicyCoverage()` added to `scripts/verify-schema-alignment.ts`, reads `baseline.sql` (a pg_dump, so it sees policies created by dynamic/looped migration SQL that a migration-text regex would miss) and flags any RLS-enabled table with no `CREATE POLICY` or without `FORCE`. Currently clean: 37/37 enabled tables forced + policied. 4 new tests in `verify-schema-alignment.test.ts`.
...
- [ ] **(code)** **`find-customer-by-name` reads out real customers' names + full phone numbers** to an unverified caller, on an unanchored `ILIKE '%…%'`. "My name is Smith" (or one common letter) returns up to 5 real `{name, phone}` pairs. Address-book enumeration where the caller supplies the only credential. **Still open** — verified 2026-08-03, unchanged. Fix: mask the phone (`•••-••-1234`) for the "is this still your number?" confirmation and require a near-exact match. **This is why it was deliberately left OUT of the toolset-wiring fix below** — wiring it in now would hand a live call a known enumeration bug.
- [x] ~~**None of the above ever ran on a live call anyway.**~~ **FIXED 2026-08-03.** All three backend fixes above were correct and complete — and entirely unreachable. Production runs question trees (`agent/src/checklist/`), and `checklistTools.ts`'s `selectedTools()` builds the model's toolset from an allowlist (`TREE_PASSTHROUGH_TOOLS`) that never included `send_verification_code`, `verify_phone_code`, or `get_customer_context` — they existed fully built in `agent/src/tools.ts` (confirmed unconditional, not capability-gated) but no call could ever reach them. A forwarded-line caller could never be recognized or verified, no matter what the backend was ready to do. Fix: added `identity: ['get_customer_context', 'send_verification_code', 'verify_phone_code']` to `TREE_PASSTHROUGH_TOOLS` — the `identity` tree is selected on every goal-bearing call. `find_caller_by_name` deliberately excluded (see item above). 2 new tests in `checklistTools.test.ts`; full agent suite (83 files / 1295 tests) + typecheck green.
