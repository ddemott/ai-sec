...
- [ ] **RAG: "address" queries don't retrieve the address doc (pre-existing, found 2026-06-23).** `./scripts/simulate.sh rag` shows "what's your address" / "could you tell me your address?" fall back to "I don't have info" instead of returning the seeded address KB doc. **Confirmed pre-existing, NOT a regression** from #39's threshold change — fails at both the old 0.5 and new 0.30 similarity thresholds (pre-#39 eval also missed it; verified by re-running the eval against `65713f1`). Root cause is content/embedding, not threshold: an address string ("123 Main Street…") embeds far from natural phrasings like "what's your address" under `text-embedding-3-small`. Options to investigate: (a) include a question-style title/alias when ingesting address/location facts so the embedding has lexical overlap, (b) a small synonym/intent expansion for common factual asks (hours/address/phone) in `shared/expandQueryForEmbedding.ts`, (c) a structured fast-path for address/hours that bypasses RAG. Low-urgency quality item; surfaced by the now-stricter 9-question eval.
