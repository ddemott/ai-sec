...
- [ ] **Fix dashboard typecheck regression in `TeamAccessView.test.tsx`** — pre-existing since `e65c833`. `mockListResponse(users: typeof ownerRow[])` is too narrow; every callsite passes a `[ownerRow, deskRow]` mixed-role array. Three errors at lines 66, 109, 161. Fix: widen the parameter to `(typeof ownerRow | typeof deskRow)[]`. CLAUDE.md still claims "Zero TypeScript errors" — that promise is currently broken on the dashboard workspace.
