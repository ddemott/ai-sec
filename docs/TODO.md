...
- [ ] **Refresh DynaTire test/seed data so it reflects the constraints we now enforce.** Surfaced 2026-05-08 by `scripts/audit-test-data.sql` after applying the booking-constraint migrations. Findings, all in DynaTire (`f234e471-...`):
  1. **3 stale appointments are still `status='scheduled'`** despite being 35–37 days in the past (2026-03-31 and 2026-04-02). Should be `completed` or `cancelled` — the in-flight `scheduled` status is just GIGO at this point.
  2. **Mike Rivera's 2026-04-02 14:00–14:30 CT appointment violates the shift-coverage invariant** the new `book_with_scheduling_atomic` enforces: he has zero `employee_schedule` rows for that week. The constraint is forward-only so this row was grandfathered in, but the data is logically inconsistent.
  3. **Two 2026-03-31 appointments have `employee_id IS NULL`** — allowed by schema, but the date has zero employee shifts in `employee_schedule` either, so they couldn't have been re-booked even with an employee assigned.
  4. **"Wrong Tenant" customer in super-admin tenant** (`00000000-...`, phone `+15555550008`, no email, created 2026-03-30 18:11:59) — almost certainly residue from an early multi-tenant isolation probe that didn't roll back. No related rows.
  5. **Bella's Hair Studio** (`bbe1d652-...`) has 1 resource and zero employees / customers / services / shifts. Either populate it as a real second test subject (would exercise the salon vertical) or delete it.
  Recommended approach: write `scripts/refresh-seed-data.sql` that (a) marks the 3 stale appts `completed`, (b) deletes the orphan customer in super-admin, (c) inserts a current-week `employee_schedule` row for Mike + a corresponding 2026-this-week appointment so the constraint gets exercised on realistic data, and (d) decides Bella's fate. Audit script (`scripts/audit-test-data.sql`) re-runnable to verify zero findings post-cleanup. Note: `supabase/seed.sql` is for fresh installs; running it against prod would truncate other tables. Targeted SQL is the right tool.
...
- [ ] **Slice 1 — backend conflict-details on overlap.** When the booking RPC returns `"already booked"`, the route does a follow-up SELECT to find the conflicting appointment (id, customer name, employee name, resource name, start/end) and returns it in the response body. New helper extracted (`src/services/conflictLookup.ts`). Both `/appointments/create` (dashboard) and `/agent-tools/book-appointment` thread the conflict block through. Unit tests pin the lookup against four overlap geometries (start, end, contained, containing) and both flavors (resource-conflict, employee-conflict).
- [ ] **Slice 1.5 — 15-min increment enforcement.** Migration adds CHECK constraint on `appointments`: `EXTRACT(MINUTE FROM start_time) IN (0,15,30,45) AND EXTRACT(SECOND FROM start_time) = 0` — same for `end_time`. Forward-only — existing rows grandfathered (the 3 in prod are already :00/:30 compliant). `AppointmentCreateSchema` Zod refinement rejects off-grid times with an `INVALID_INCREMENT` shape. Same Zod refinement on the agent-tools booking schema. Tests pin happy + sad for both surfaces; backend and DB layer.
- [ ] **Slice 2 — dashboard conflict modal + 15-min time picker.** When `/appointments/create` returns the new conflict block, surface a modal: "This time is taken: {Customer} with {Employee} from {start} to {end}", with View / Pick another time CTAs. Time picker (in QuickBookPanel + AppointmentDetailPanel) becomes a dropdown of valid 15-minute options instead of free-form input. Component tests cover modal render under conflict, modal dismissal, time picker option set.
- [ ] **Slice 3 — E2E with self-contained data lifecycle.** New `dashboard/e2e/helpers/fixtures.ts` exports `seedTenantData()` / `cleanTenantData()` so each spec creates its own employees, resources, schedules, appointments in `beforeEach` and tears them down in `afterEach`. Per the test-isolation memory: each test runs independently, owns its full data lifecycle. Four scenarios: (a) out-of-hours blocked + correct error; (b) employee double-book blocked + conflict modal shows existing appt; (c) resource double-book blocked + conflict modal; (d) partial-overlap blocked (proves whole-slot check, not just edges).
- [ ] **AI prevention — prompt-only enforcement (deferred).** Tighten `agent/src/prompt.ts` system prompt with a hard rule: agent MUST call `/agent-tools/check-availability` (or `/agent-tools/available-slots` / `/agent-tools/scheduling-options`) before `/agent-tools/book-appointment`; NEVER propose a specific time without verifying it's free first. Pin with conversation-shape tests that fail if the agent skips the availability check before booking. Cheapest fix — agent already has the tools, just needs prompt discipline.
