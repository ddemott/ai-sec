...
- [x] **[dev]** **Twilio delivery monitoring** — DONE 2026-06-12 (`feat/twilio-delivery-receipts`): `TwilioAdapter.sendSMS` now attaches a `statusCallback` (`BACKEND_PUBLIC_URL`-gated, omitted when unset) pointing at the new `POST /communications/twilio/status` webhook, which verifies `X-Twilio-Signature`, upserts into the new `message_delivery_status` table by SID, and increments `message_delivery_receipts_total{status}`. Needs `BACKEND_PUBLIC_URL` set on Railway to activate.
