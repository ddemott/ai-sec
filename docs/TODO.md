...
- [ ] **Cluster B — verified defects** (3 sites, independent fixes)
  ...
  - [ ] `SetupWizard/index.tsx:101` — template seeding is a sequential `await Api.services.create` loop; mid-loop failure (catch at :118 only `console.warn`s) leaves setup half-seeded with no UI status or retry. **Design note (found 2026-05-21):** the seed is gated on `services.length === 0`, so a partial failure can NOT be retried (length is now >0 → guard skips the rest). A correct fix needs (a) **reconcile by name-diff** — create only the missing `example_services`, not all-or-nothing; (b) surface seeding status + error + a Retry action in the wizard render; (c) a test. Genuinely focused work, not a one-liner.
