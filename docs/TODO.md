...
- [x] **Test RAG accuracy — DONE 2026-06-12.** `scripts/sim-rag.mjs` + `./scripts/simulate.sh rag` — seeds a known KB into a demo tenant (real embeddings via `/knowledge/add`), asks paraphrased caller questions through `/agent-tools/policy-answer`, grades retrieval (expected content present, + out-of-scope must fall back not hallucinate), reports a hit-rate and exits non-zero below 80%. On-demand quality tool (real OpenAI → not a CI gate; non-deterministic + costs). Run-verified: **5/6 (83%)**. **Gates the website-scan onboarding idea** (`docs/STRATEGY.md`).
  - [x] **Finding from the eval — FIXED 2026-06-12.** *"what's your address"* fell back instead of retrieving the location doc — `address`↔`located` shares no vocabulary and scored 0.31, below the 0.5 threshold. **Root cause measured** (text-embedding-3-small, real embeddings): the reductive `normalizeForEmbedding` applied to the *query* made it WORSE — it collapsed "what's your address" → "Address inquiry" (0.215), below even true out-of-scope questions, i.e. unseparable by any threshold. **Fix:** new `shared/expandQueryForEmbedding.ts` (additive synonym expansion, the inverse of normalization) on the policy-answer query path only + threshold 0.5→0.30, validated against a widened eval set (8 paraphrased positives incl. two address phrasings + 3 true out-of-scope). Docs/ingest untouched → no re-embed. Run-verified: **sim-rag 9/9 (100%)**. See `shared/expandQueryForEmbedding.ts` + `src/queryExpander.test.ts`.
