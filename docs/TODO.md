...
- [ ] **(code)** **Finish the dashboard component subdirectory migration** — **57** loose `.tsx` files at `dashboard/components/` (count 2026-08-29: was 61 after team slice). Seventh slice: AuditLogView, RecordHistoryModal → `components/records/`.
...
- [ ] Continue count-drift passes (route modules / migrations / test numbers) after any new route or migration; keep secondary docs synced. **2026-08-29 pass:** dashboard loose `.tsx` at `components/` is **57** (records slice); `supabase/migrations/` still **189**.
