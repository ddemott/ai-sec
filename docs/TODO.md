...
These branches survived the 2026-06-23 cleanup because each holds real unmerged work that hit a hard gate (prod migration / legal / active-work collision). Captured here so the intent isn't lost if a branch is ever pruned. Triage method + full context: `RESOLVED.md` 2026-06-23 + memory `project_branch_cleanup_2026_06_23`.

- [ ] **`feat/default-appointment-buffer` (PR #41) — per-tenant default buffer between appointments.** Adds `src/services/tenantBuffer.ts` + `getTenantBufferMinutes()` (read in agent booking tools), a buffer UI in `BusinessSettingsView`, and 2 migrations: `20260607000000_tenants_default_buffer` (column) + `20260607000001_booking_buffer_enforcement` (627-line booking-RPC change enforcing the gap). **Genuinely missing from main — the real keeper.** To land: (1) apply both migrations to prod FIRST (merge-before-migrate; the RPC is booking-critical), (2) review the enforcement RPC vs current `book_with_scheduling_atomic`, (3) `git merge origin/main` + resolve (`BusinessSettingsView` JSX, keep both agentTools imports) + `npm run db:baseline`, (4) full tests → merge. **Blocked on:** prod migration + RPC review (owner-led). Memory: [[project_default_buffer]].
