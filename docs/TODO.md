...
- [ ] **Clean up `any` types in backend test files — 77 remaining (down from 215).** Earlier sweeps cleared 86 across `33f83cd` + `01b7009` (2026-05-04). 2026-05-05 cleared 17 in `src/middleware-helpers.test.ts` using `as unknown as AppRequest` / `as unknown as FastifyReply` for partial-mock structural casts. 2026-05-06 cleared 41 across five files (normalizer 12, provisioning 10, coverage 7, auth 7, routeHelpers 5) — verified `bugfix-comprehensive.test.ts` had zero real instances (the prior 11 were all comment matches in 5W headers, an artifact of imprecise regex). Remaining 77 across ~25 files; top of the queue: servicetitan-sync (6), middleware (6), high-bugs (5), square-sync (4), jobber-sync (4). Pattern reference: `vi.mocked(mockFn)` for typed mock access, `as unknown as Type` for partial-mock structural casts, proper `Fastify`/`Pool` type imports for production-shape mocks. Define a row type and use `client.query<RowType>(...)` for pg query-result rows instead of `(r: any)` callbacks.
