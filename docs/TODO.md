...
- [x] **[dev]** **Transfers invisible in Calls tab** — agent `recordTransfer()` + outcome plumbing + `end_voice_session` RPC already accepted 'transferred' outcome; the missing piece was the RPC forcing status='completed'. Fixed via migration + function logic (status = CASE WHEN outcome='transferred' THEN 'transferred' ELSE 'completed'). UI (VoiceCallsView badges/filters, shared types) already supported it. Tests + psql integration verify. See feat/transfers-invisible-calls.
...
- [x] **Generate + send call summary** — implemented: `agent/src/callSummary.ts` (bounded summarizeCall) + called in index.ts shutdown + passed to voice-session-end (and displayed in VoiceCallsView). Part of the voice-session capture shipped work.
...
- [x] **Register transfer events in the call record** — Done via `recordTransfer()` (sets outcome) + updated `end_voice_session` (sets status='transferred' when outcome=transferred). The call now lands correctly in Calls tab. (pino logs remain for ops.) See the transfers task.
