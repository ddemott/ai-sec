...
- [ ] **(code)** **Finish the dashboard component subdirectory migration** — **71** loose `.tsx` files at `dashboard/components/` (count 2026-08-28: was 75 after auth slice). Third slice: AppShell, OutlookLayout → `components/layout/`.
...
- [ ] Continue count-drift passes (route modules / migrations / test numbers) after any new route or migration; keep secondary docs synced. **2026-08-28 pass:** dashboard loose `.tsx` at `components/` is **71** (layout shell slice); `supabase/migrations/` still **189**.
