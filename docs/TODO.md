...
- [ ] **`feat/knowledge-suggestions` (PR #42) — improved variant of the website-scan Q&A review UI.** The base feature already shipped to main (`00a9158`); this branch is a _better_ version: matched items enter as `'suggested'` (owner reviews everything before live KB), the approve path is wrapped in a try/catch txn with ROLLBACK + 409 guard (main can orphan a doc on 0-row update), and AI-cost is recorded on approve. **Blocked on:** overlaps Grok's active `feat/website-knowledge-import` (same `knowledge.ts` handler) — fold these improvements into Grok's work rather than merging a competing version. Backend test fix already pushed (`a41d576`); E2E website-scan still red (stub count).
...
- [ ] **`fix/agent-tenant-resolution` (no PR) — go-live operational docs** (`GO_LIVE_FINDINGS.md` + 289-line `TELNYX_HANDOVER.md`, both new; not in main). Despite the `fix/` name the unmerged commits are docs only. **To resolve:** diff against current `docs/RUNBOOK.md` / `BETH_GO_LIVE_TODO.md` (which now cover much of the Telnyx ops detail) → merge the unique delta or close as superseded.
...
- [ ] **RAG: "address" queries don't retrieve the address doc (pre-existing, found 2026-06-23).** `./scripts/simulate.sh rag` shows "what's your address" / "could you tell me your address?" fall back to "I don't have info" instead of returning the seeded address KB doc. **Confirmed pre-existing, NOT a regression** from #39's threshold change — fails at both the old 0.5 and new 0.30 similarity thresholds (pre-#39 eval also missed it; verified by re-running the eval against `65713f1`). Root cause is a vocabulary gap, not threshold: the seeded doc is a Q/A pair (`"Where are you located?"` / `"We are located at 123 Main Street in downtown."`) which shares no words with the caller phrasing "what's your **address**" — so the cosine similarity stays below even the new 0.30 floor under `text-embedding-3-small`. **Measured diagnosis (2026-06-23, real `text-embedding-3-small` cosines via a standalone probe replicating the prod ingest+query path):** every other positive scores 0.59–0.63 (hours 0.60, oil 0.59, cancel 0.61, tire-rotation 0.63); both address phrasings score **0.302**; all out-of-scope negatives score ≤0.20 (hamburgers 0.154, refrigerator 0.200, wifi 0.122). So address is an **outlier-low positive**, not a threshold problem per se. Two contributing causes: (1) the expander emits "address location situated coordinates place" — "location" but **not the doc's actual word "located"** (so it misses the lexical match; it even _lowered_ the terse query 0.312→0.302); (2) the ingest normalizer reduces the Q/A pair to just "Located at 123 Main Street in downtown." — **dropping the question form "Where are you located?"**, which is exactly the retrieval signal that would match "what's your address". The RPC `search_tenant_docs_normalized` uses strict `> p_match_threshold`, so at 0.30 the address case (0.302) clears by only 0.002 → run-to-run embedding variance flips it (the probe got 0.302 = hit while the live eval missed on the same input → nondeterministic at the boundary).

  **Do NOT just lower the global threshold.** The current failure (fall back → "let me take a message") is the SAFE failure. Lowering the cutoff to fit this 5-doc eval would, on a real tenant with hundreds of dense near-neighbor docs, surface a marginally-related chunk → the LLM answers **confidently from the wrong doc** on a live call — strictly worse for a receptionist than taking a message. The 5-doc synthetic eval structurally cannot detect this (well-separated docs always leave negatives low). Any threshold change must be validated on a **real tenant corpus** (or by flip-rate over N eval runs, since one green pass is a coin landing heads — not verification).

  **Durable fix = doc-side (the real one):** keep the question form in the embedded text for short factual Q/A (hours/address/phone) — e.g. embed the question + answer rather than the normalizer's reduced answer-only — so the doc vector carries "where/located". This needs a **re-embed migration** (changing ingest strands existing vectors), which is why it's gated. Lighter query-side palliative (smaller blast radius, but LLM-nondeterministic + can introduce its own false positives): tighten the expander prompt to emit morphological/doc variants (address→located/locate/where). **Owner's call on which path** — all three (threshold / expander / doc-side re-embed) trade off live-voice accuracy; surfaced with numbers, not auto-applied. Low-urgency.
