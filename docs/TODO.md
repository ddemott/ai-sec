...
- **OPEN, needs Dale's ear, not mine:** flag is on. Remaining question is whether the filler *sounds* like cover and not a stutter on a real call (2800ms deadline, `HOLD_LINE`). Place one real test call; keep or set `ENABLE_OUTPUT_WATCHDOG=false`. CI cannot grade this.
...
- [ ] **(code)** **Finish the dashboard component subdirectory migration** — **38** loose `.tsx` files at `dashboard/components/` (count 2026-08-29: was 53 after crm slice). Ninth slice: AppointmentView family → `appointments/`; KnowledgeBaseView, KnowledgeSuggestions, ExplainAnswerView → `knowledge/`; AnalyticsView, UtilizationHeatmap, AIInsightsView → `analytics/`.
...
- [ ] Continue count-drift passes (route modules / migrations / test numbers) after any new route or migration; keep secondary docs synced. **2026-08-29 pass:** dashboard loose `.tsx` at `components/` is **38** (appointments/knowledge/analytics slice); `supabase/migrations/` still **189**.
