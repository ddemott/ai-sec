...
- [ ] **(code)** **Finish the dashboard component subdirectory migration** — **61** loose `.tsx` files at `dashboard/components/` (count 2026-08-29: was 66 after billing slice). Sixth slice: SuperAdminDashboard, TenantCard, TenantCreateForm, TenantEditPanel → `components/team/`.
...
- [ ] Continue count-drift passes (route modules / migrations / test numbers) after any new route or migration; keep secondary docs synced. **2026-08-29 pass:** dashboard loose `.tsx` at `components/` is **61** (team slice); `supabase/migrations/` still **189**.
