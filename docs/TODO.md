...
- [ ] **Owner-facing UI to invite users + assign role.** Today the only way to create a `front_desk` user is via SQL. For DynaTire's beta this is fine (Dale provisions a single front-desk login by hand if needed) but it's a launch-blocker for any second customer. Scope: a "Team" sub-section under Back Office with invite-by-email + role dropdown, plus a `POST /users/invite` route + email template. Decide before opening the beta funnel beyond DynaTire.
