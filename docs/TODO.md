...
- [ ] **Pure-inquiry abandonment tracking** (carved out of "Analytics depth" 2026-07-02 — the one remaining edge). Callers who only asked availability and never attempted a booking aren't counted as abandonment: the `available-slots` / `scheduling-options` agent tools don't carry `call_id`, so there's no signal to attribute to the `voice_session`. Needs an agent-worker change to thread `call_id` into those tools + a best-effort `voice_sessions` write (mirrors how `book-with-scheduling` already captures `requested_service_id`). Booking-attempt abandonment (by service) is already tracked.
