...
**Status:** active execution. Initial block/compiler + generic-intake checkpoint is implemented; rollout steps beyond that remain open. **Date:** 2026-08-11. **Owner:** Dale.
...
## Checkpoint completed on 2026-08-11
...
- a clean projector/service extraction beyond the current route-level implementation
- a second reusable intake path
...
Best candidates:
- `buy_service`
- estimate/service-intake flow

Recommendation:
- use `buy_service` for faster proof of concept
- use service-intake/estimate flow for stronger vertical-value proof
...
1. split `job_inquiry` more cleanly into generic capture + projector/service boundaries
2. onboard a second reusable path through the same compiler/runtime shape
3. define the first three real vertical presets
4. wire preset selection into setup/onboarding
5. add tenant-safe overrides and preview/activation guardrails
