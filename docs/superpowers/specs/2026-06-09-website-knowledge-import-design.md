...
> **⚠️ SUPERSEDED (2026-06-29) — kept as design history.** The DB-backed
> question-bank approach described here was NOT taken. The website-import question
> bank shipped via **PR #45** (`shared/questionBank.ts`) as a **TS constant**, with
> no `question_bank` DB table, and **business-type filtering was intentionally
> dropped** (all categories are shown to every tenant — see the header of
> `shared/questionBank.ts`). This document records the original 2026-06-09 thinking;
> it does not describe current behavior. Source of truth: `shared/questionBank.ts`.

**Date:** 2026-06-09
**Status:** SUPERSEDED by PR #45 (was: approved design, pending implementation plan)
