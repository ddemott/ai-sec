...
-- The bank tables are SHARED reference data (no tenant_id). Mirror the
-- business_templates RLS convention so they aren't the lone RLS-off tables in a
-- schema that FORCEs RLS everywhere: publicly readable + admin (no-tenant) bypass.
ALTER TABLE question_bank ENABLE ROW LEVEL SECURITY;
ALTER TABLE question_business_type ENABLE ROW LEVEL SECURITY;

DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies
                   WHERE tablename = 'question_bank' AND policyname = 'Publicly readable question bank') THEN
        CREATE POLICY "Publicly readable question bank" ON question_bank FOR SELECT USING (true);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies
                   WHERE tablename = 'question_bank' AND policyname = 'admin_bypass_question_bank') THEN
        CREATE POLICY admin_bypass_question_bank ON question_bank
            USING (NULLIF(current_setting('app.current_tenant_id', true), '') IS NULL);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies
                   WHERE tablename = 'question_business_type' AND policyname = 'Publicly readable question_business_type') THEN
        CREATE POLICY "Publicly readable question_business_type" ON question_business_type FOR SELECT USING (true);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies
                   WHERE tablename = 'question_business_type' AND policyname = 'admin_bypass_question_business_type') THEN
        CREATE POLICY admin_bypass_question_business_type ON question_business_type
            USING (NULLIF(current_setting('app.current_tenant_id', true), '') IS NULL);
    END IF;
END
$$;
...
Both follow-on plans are now written to full bite-sized detail:

- **Plan 2 — Scrape + Extract engine:** `2026-06-09-website-knowledge-import-plan2-scrape-extract.md`. `knowledge_suggestion` table, bounded same-origin crawl (`websiteScrape.ts`), single-LLM extraction with prompt-injection containment (`answerExtraction.ts`), `POST /knowledge/import-website` orchestration. Depends on this plan's `resolveQuestionSet` + `tenant_custom_question`.
- **Plan 3 — Suggestion review UI + approve→embed:** `2026-06-09-website-knowledge-import-plan3-review-ui.md`. List/approve/reject endpoints (approve embeds via `prepareQADocument`), new `StepWebsiteImport` wizard step before caller-questions (team + solo), suggestion-aware questionnaire (badge + confidence + empty-vs-found + discovered). Depends on Plans 1 and 2.
