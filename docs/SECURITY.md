...
- 29 multi-tenant isolation probes (`src/multi-tenant-isolation.test.ts`) run on every CI build against real Postgres, exercising query-string override + body-FK injection + cross-tenant id under JWT-only + admin-route gating + RLS configuration metadata.
