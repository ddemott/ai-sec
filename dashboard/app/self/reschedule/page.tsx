...
export default function SelfReschedulePage({
  ...
  const handleRequest = async () => {
    ...
    try {
      ...
      if (data.success) {
        setState('success');
        setMessage(data.message ?? "Your reschedule request has been sent. We'll contact you shortly.");
      } else {
        setState('error');
          ...
          {state === 'confirm' && (
            <>
              <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>
                Tap below to notify us that you&apos;d like to reschedule. We&apos;ll contact you
                to find a new time that works.
              </p>
              <button
