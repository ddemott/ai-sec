...
const mockCall = {
  ...
  customer_context: {
    ...
    notes: [
      { id: 'note-1', text: 'Prefers morning appointments', type: 'preference', created_at: '2024-03-01T00:00:00Z' },
    ],
    tags: ['VIP', 'Regular'],
  },
}
...
describe('VoiceCallsView', () => {
  ...
  describe('Happy Paths', () => {
    ...
    test('displays call summary when available', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText('Customer booked a 30-minute service appointment.')).toBeInTheDocument()
      })
  ...
  describe('Sad Paths', () => {
    ...
    test('displays new caller message for unknown customers', async () => {
      ...
      mockCallHistory.mockResolvedValue({ calls: [unknownCustomerCall], total: 1, has_more: false })
      render(<VoiceCallsView />)
      await waitFor(() => {
  ...
  describe('Edge Cases', () => {
    ...
    test('handles all outcome types correctly', async () => {
      const outcomes = ['appointment_booked', 'appointment_rescheduled', 'appointment_cancelled', 'info_provided', 'transferred', 'voicemail', 'abandoned', 'other']
      const labels = ['Booked', 'Rescheduled', 'Cancelled', 'Info Provided', 'Transferred', 'Voicemail', 'Abandoned', 'Other']
