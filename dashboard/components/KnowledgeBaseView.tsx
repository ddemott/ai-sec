...
import {
  ...
  Search, Loader2, ChevronDown, ChevronRight, MessageSquare, Save, Plus, X
} from 'lucide-react'
  ...
  return (
    <div className="border rounded-xl overflow-hidden" style={{ borderColor: 'var(--border-soft)' }}>
      ...
      >
        <div className="flex items-center gap-3">
          {open ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
          <span className="font-medium" style={{ color: 'var(--text-primary)' }}>Custom Questions</span>
        </div>
        <Badge variant="secondary">{customDocs.length}</Badge>
      </button>
      {open && (
        <div className="p-4 border-t space-y-3" style={{ borderColor: 'var(--border-soft)', backgroundColor: 'var(--bg-surface)' }}>
          <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
            Add business-specific Q&amp;A the AI should know — things not covered by the preset categories above. The agent will use these the same way it uses preset answers.
          </p>

          {/* Add form (toggled) */}
          {showForm ? (
            <Card className="p-3 space-y-2">
              <label className="block text-xs font-bold uppercase" style={{ color: 'var(--text-secondary)' }}>Question</label>
              <textarea
                value={newQ}
              ...
              />
              <label className="block text-xs font-bold uppercase pt-2" style={{ color: 'var(--text-secondary)' }}>Answer</label>
              <textarea
                value={newA}
              ...
              <div className="flex justify-end gap-2 pt-1">
                <Button variant="ghost" size="sm" onClick={cancelForm} disabled={saving}>Cancel</Button>
                <Button
                  variant="primary"
                  size="sm"
                  onClick={handleAdd}
          ...
          {customDocs.length === 0 && !showForm && (
            <p className="text-sm italic" style={{ color: 'var(--text-muted)' }}>No custom questions yet.</p>
          )}
          {customDocs.map(doc => {
            ...
            return (
              <Card key={doc.tenant_doc_id} className="p-3 group relative">
                ...
                <div className="text-sm font-medium mb-1 pr-8" style={{ color: 'var(--text-primary)' }}>{q}</div>
                <div className="text-sm whitespace-pre-wrap" style={{ color: 'var(--text-secondary)' }}>{a}</div>
              </Card>
            )
          })}
        </div>
            ...
            {tab === 'questionnaire' && (
              <div className="space-y-3 max-w-3xl pb-8">
                <p className="text-sm mb-4" style={{ color: 'var(--text-secondary)' }}>
                  ...
                  Answers auto-save as you type. Add business-specific Q&amp;A in the Custom Questions section at the bottom.
                </p>
