...
export default function ShiftManagementView() {
  ...
  async function fetchShifts() {
    ...
    try {
      ...
      const data = await Api.shifts.schedule.forDate(tenantId, selectedEmployeeId, toDateStr(weekStart), toDateStr(endDate))
        ...
        {selectedEmployeeId && (
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-2">
              <button onClick={prevWeek} className="p-1.5 rounded-lg transition-colors" style={{ color: 'var(--text-secondary)', backgroundColor: 'var(--bg-raised)' }}><ChevronLeft className="w-4 h-4" /></button>
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-0 rounded-lg overflow-hidden" style={{ border: '1px solid var(--border-soft)' }}>
                <button onClick={handleZoomOut} disabled={colW <= MIN_COL_W} className="p-1.5 transition-colors disabled:opacity-30" style={{ color: 'var(--text-secondary)' }}><Minus className="w-3.5 h-3.5" /></button>
                <span className="text-xs font-bold px-2 select-none" style={{ color: 'var(--text-muted)' }}>{getZoomPercent(colW)}%</span>
                <button onClick={handleZoomIn} disabled={colW >= MAX_COL_W} className="p-1.5 transition-colors disabled:opacity-30" style={{ color: 'var(--text-secondary)' }}><Plus className="w-3.5 h-3.5" /></button>
              </div>
            </div>
          </div>
        )}
        ...
        {!selectedEmployeeId ? (
          ...
          <div className="h-full flex flex-col rounded-2xl border overflow-hidden relative" style={{ backgroundColor: 'var(--bg-raised)', borderColor: 'var(--border-soft)' }}>
            ...
            <div className="flex shrink-0" style={{ borderBottom: '1px solid var(--border-soft)' }}>
              <div className="shrink-0 flex items-center px-3 text-[10px] font-bold uppercase tracking-widest" style={{ width: DAY_LABEL_WIDTH, height: HEADER_HEIGHT, color: 'var(--text-muted)', borderRight: '1px solid var(--border-soft)' }}>Date</div>
              <div ref={headerRef} className="flex-1 overflow-hidden">
                <div className="flex" style={{ width: totalGridWidth }}>
                  {HOURS.map(h => (
                    <div key={h} className="text-center text-[10px] font-bold shrink-0 flex items-center justify-center select-none" style={{ width: colW, height: HEADER_HEIGHT, color: 'var(--text-muted)', background: h < DEFAULT_OPEN_HOUR || h >= DEFAULT_CLOSE_HOUR ? 'rgba(0,0,0,0.2)' : 'transparent', borderRight: '1px solid var(--border-soft)' }}>
                      {formatHour(h)}
                    </div>
                  ))}
                </div>
              ...
              <div className="shrink-0" style={{ width: DAY_LABEL_WIDTH }}>
                {weekDays.map(day => (
                  <div key={day.dateStr} className="flex items-center px-3 cursor-pointer hover:brightness-110 transition-all" style={{ height: ROW_HEIGHT, borderBottom: '1px solid var(--border-soft)', borderRight: '1px solid var(--border-soft)', backgroundColor: day.isToday ? 'rgba(59,130,246,0.08)' : undefined }} onClick={() => openEditor(day.dateStr)}>
                    <span className="font-bold text-sm" style={{ color: day.isToday ? 'var(--accent, #3b82f6)' : shiftForDate(day.dateStr) ? 'var(--text-primary)' : 'var(--text-muted)' }}>{day.label}</span>
                  </div>
                ))}
              </div>
              ...
              <div ref={gridRef} className="flex-1 overflow-x-auto" onScroll={handleGridScroll}>
                <div style={{ width: totalGridWidth }}>
                  {weekDays.map(day => {
                    ...
                    return (
                      <div key={day.dateStr} className="relative" style={{ height: ROW_HEIGHT, borderBottom: '1px solid var(--border-soft)', backgroundColor: day.isToday ? 'rgba(59,130,246,0.04)' : undefined }}>
                        <div className="absolute inset-0 flex">
                          {HOURS.map(h => (
                            <div key={h} className="shrink-0" style={{ width: colW, height: ROW_HEIGHT, background: h < DEFAULT_OPEN_HOUR || h >= DEFAULT_CLOSE_HOUR ? 'rgba(0,0,0,0.15)' : 'transparent', borderRight: '1px solid var(--border-soft)' }} />
                          ))}
                        </div>

                        {hasShift && (
                          <div className="absolute group cursor-pointer rounded-md transition-all hover:brightness-110" style={{ left: shiftTimeToHour(shift.start_time!) * colW, width: Math.max((shiftTimeToHour(shift.end_time!) - shiftTimeToHour(shift.start_time!)) * colW, 8), top: 6, bottom: 6, background: 'var(--accent, #3b82f6)', opacity: 0.85, zIndex: 2 }} onClick={() => openEditor(day.dateStr)} title={`${formatTime24to12(shift.start_time!.substring(0, 5))} - ${formatTime24to12(shift.end_time!.substring(0, 5))}`}>
                            {(shiftTimeToHour(shift.end_time!) - shiftTimeToHour(shift.start_time!)) * colW > 90 && (
                        ...
                        {shift?.is_off && (
                          <div className="absolute inset-0 flex items-center justify-center" style={{ zIndex: 2 }}>
                            <span className="text-[10px] font-bold px-2 py-0.5 rounded" style={{ backgroundColor: 'rgba(239,68,68,0.15)', color: '#ef4444' }}>OFF</span>
                          </div>
                        )}

                        {!shift && (
                          <div className="absolute inset-0 flex items-center justify-center cursor-pointer" style={{ zIndex: 2 }} onClick={() => openEditor(day.dateStr)}>
                            <span className="text-[10px] italic" style={{ color: 'var(--text-muted)' }}>Click to schedule</span>
                          </div>
                        )}
                      </div>
                    )
