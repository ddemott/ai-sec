...
describe('useSkillMapData', () => {
  ...
  test('selection highlights connected nodes transitively', async () => {
    ...
    act(() => {
      result.current.selectNode('emp-emp-uuid-1');
    });
...
describe('completeLinking', () => {
  ...
  test('assigns employee to service via mapping API', async () => {
    ...
    act(() => {
      result.current.startLinking('emp-emp-uuid-2', 'employee');
    });
    ...
    expect(Api.mappings.assignServiceEmployee).toHaveBeenCalledWith(
      'svc-uuid-1',
      'emp-uuid-2',
      'test-tenant'
    );
  ...
  test('assigns resource to service via mapping API', async () => {
    ...
    act(() => {
      result.current.startLinking('res-res-uuid-2', 'resource');
    });
    ...
    expect(Api.mappings.assignServiceResource).toHaveBeenCalledWith(
      'svc-uuid-2',
      'res-uuid-2',
      'test-tenant'
    );
  ...
  test('does not duplicate existing assignment', async () => {
    ...
    act(() => {
      result.current.startLinking('emp-emp-uuid-1', 'employee');
    });
    await act(async () => {
      await result.current.completeLinking('skill-svc-uuid-1');
    });
  ...
  test('linking from service shows employees and resources as valid targets', async () => {
    ...
    act(() => {
      result.current.startLinking('skill-svc-uuid-3', 'skill');
    });
...
describe('disconnectConnection', () => {
  ...
  test('unassigns employee from service via mapping API', async () => {
    ...
    expect(Api.mappings.unassignServiceEmployee).toHaveBeenCalledWith(
      'svc-uuid-1',
      'emp-uuid-1',
      'test-tenant'
    );
  ...
  test('unassigns resource from service via mapping API', async () => {
    ...
    expect(Api.mappings.unassignServiceResource).toHaveBeenCalledWith(
      'svc-uuid-1',
      'res-uuid-1',
      'test-tenant'
    );
