...
describe('SuperAdminDashboard sad paths', () => {
  ...
  test('shows error when tenant creation fails', async () => {
    const fetchMock = vi.fn<typeof fetch>(async (input, init) => {
      ...
      if (url.endsWith('/templates')) {
        return mockJsonResponse([
          { business_type: 'mobile-tire', display_name: 'Mobile Tire Shop' },
        ]);
      }
      if (url.endsWith('/tenants/create') && init?.method === 'POST') {
    ...
    // Fill minimal form
    fireEvent.change(screen.getByPlaceholderText(/Elite Salon/i), {
      target: { value: 'Duplicate Biz' },
    });
    ...
    fireEvent.change(screen.getByPlaceholderText(/Owner Password/i), {
