...
async function assignMappings(
    req: APIRequestContext, token: string, tenantId: string,
    serviceId: string, employeeId: string, resourceId: string,
): Promise<void> {
    const e = await req.post(
        `${BACKEND_URL}/services/${serviceId}/employees/${employeeId}/assign`,
        { headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, data: { tenant_id: tenantId } },
    );
...
for (const c of INDUSTRY_CASES) {
    test(`${c.industry}: register → vocabulary → finalize → book`, async ({ request }) => {
        ...
        try {
            ...
            const vocabRes = await request.get(
                `${BACKEND_URL}/vocabulary?tenant_id=${tenant.tenantId}`,
            ...
            const resourceId = await createResource(request, tenant.token, tenant.tenantId, 'Sample Resource');
            const employeeId = await createEmployee(request, tenant.token, tenant.tenantId, tenant.ownerName);
            ...
            if (tenant) {
                ...
                await pool.query('DELETE FROM service_employee WHERE tenant_id = $1', [tenant.tenantId]).catch(() => { /* ok */ });
