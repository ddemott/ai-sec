...
**Phase 13 (Production Readiness) in progress.** ~1,940 backend + ~790 dashboard + ~360 agent tests passing (verified 2026-06-22 against real test_db / vitest). 29 route modules under `src/routes/`. Playwright e2e specs run on every PR. Zero TS errors across backend / agent / dashboard. **Shipped + merged to main + DEPLOYED to prod this session (2026-06-22), PRs #56/#57/#58/#59** (verified live via `./scripts/simulate.sh status --env prod --deep` = 4/4 + the new routes return 401 not 404 on prod): idempotent-read retry coverage in `toolsClient`; tenant-data export `GET /export/tenant-data`; per-tenant website-scan rate-limit; incident + telephony runbook (`docs/RUNBOOK.md`); owner audit-log API `GET /audit-log`; "explain this answer" RAG-debugger `POST /knowledge/explain`; owner admin guide (`docs/OWNER_GUIDE.md`); dashboard surfaces for audit-log + answer-debugger (Setup sub-tabs) + a "Download my data" export button; caller-facing source citations in `policy-answer` (agent prompt updated); website-scan happy-path + wizard browser-click E2E (stub-gated). **No prod DB migration was needed** (all read existing schema). Coverage breakdown in `docs/TEST_COVERAGE.md`; security posture in `docs/SECURITY.md`; Railway + Sentry + Better Stack setup in `docs/DEPLOYMENT.md`. Session handoff: `docs/HANDOFF.md`.

**Also shipped + merged this session (2026-06-22), PRs #64/#65/#66/#67** (merged → deployed): abandonment-by-service analytics (migration `20260622010000` adds `voice_sessions.requested_service_id`; the `book-with-scheduling` tool best-effort captures the requested service whether the booking succeeds or fails; `/analytics/cohorts` returns `abandonment_by_service`); **optional From/To date-range filtering** on `/analytics/calls` + `/analytics/cohorts` (new `optionalDateBounds` — all-time when absent, calendar-invalid dates rejected to null via `isValidDateOnly`, end day-inclusive; From/To controls on `AnalyticsView`); `@typescript-eslint/unbound-method` promoted `warn → error` in all 3 eslint configs (0 violations). **Two PRs (#68/#69) CLOSED 2026-06-23 in the branch cleanup (branches deleted; restorable from the PR page), still held for owner/legal review — do NOT merge/enable without sign-off**: `POST /customers/:id/purge` GDPR/CCPA single-customer erasure (owner-gated, typed phone confirmation, atomic anonymize-in-place + audit_log PII redact, **runtime kill-switch `ENABLE_CUSTOMER_PURGE` — inert/404 until explicitly enabled**) and a disabled-by-default automated data-retention worker (`ENABLE_RETENTION_WORKER` + an explicit `RETENTION_DAYS`, no default window, anonymize-in-place). Both flagged because they erase PII irreversibly; broader-PII scope (voice_sessions/transcripts/appointment descriptions) is a deliberate follow-up. **Prod migrations: DONE 2026-06-23** — `20260622000000` (audit-extend) + `20260622010000` (requested_service_id) applied + verified on prod (`APPLIED=2`; `requested_service_id` column + `trg_audit_services`/`trg_audit_employees` triggers present; `schema_migrations` at `20260622010000`).
...
Remaining blockers: PSTN inbound path unverified — different-carrier call to `+1 630-822-9086` while watching `listRooms()`; `+1 630-822-9086` is the live number. Live call-transfer (`transfer_call`) additionally needs **call transfer / REFER enabled on the Telnyx SIP Connection**. `DASHBOARD_URL` + `SENTRY_DSN` on Railway, prod migrations apply (incl. `20260611000000_tenant_forward_phone`). See `docs/AIASSISTANT_GO_LIVE_TODO.md` + `docs/TODO.md`.
