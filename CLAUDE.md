...
- `/supabase/migrations` — 137 SQL migrations (incl. 20260616 customer_messages + 20260615 preferences_default_true + 20260612 knowledge_suggestion + comms history + voice styles + forward phone).
- `/scripts` — `simulate.sh` (system simulation/health harness + new `ci` subcommand for build/CI pipeline visibility; node helpers in `agent/scripts/sim-*.mjs` for LiveKit + `scripts/sim-tools.mjs` for the agent-tools journey), `verify-claude-md.ts` drift detector
