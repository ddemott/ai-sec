...
- `/supabase/migrations` — 85 SQL migrations.
...
## Project Status
**Phase 13 (Production Readiness) in progress.** 1,704 backend + 613 dashboard = 2,317 tests passing (verified 2026-05-08; 0 skips). 78 agent tests, 42 Playwright e2e (1 skipped), 29 live QA tool calls. Zero TS errors across backend / agent / dashboard. Detailed coverage breakdown — including V8 percentages and e2e workflow inventory — lives in `docs/TEST_COVERAGE.md`; refresh it whenever a commit measurably moves test counts or coverage.
