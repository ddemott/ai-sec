...
- `/supabase/migrations` — 141 SQL migrations (incl. 20260622 audit_services_employees + 20260619 fix_rls_gaps + 20260618 ai_cost_events + ai_cost_events_rls + 20260616 customer_messages + 20260615 preferences_default_true + 20260612 knowledge_suggestion + comms history + voice styles + forward phone). **`supabase/baseline.sql` is a generated pg_dump snapshot — regenerate with `npm run db:baseline` after any schema migration** (the schema-alignment guard fails CI when a migration-created table/column is missing from it).
