...
**Also shipped + merged this session (2026-06-22), PRs #64/#65/#66/#67** (merged → deployed): abandonment-by-service analytics (migration `20260622010000` adds `voice_sessions.requested_service_id`; the `book-with-scheduling` tool best-effort captures the requested service whether the booking succeeds or fails; `/analytics/cohorts` returns `abandonment_by_service`); **optional From/To date-range filtering** on `/analytics/calls` + `/analytics/cohorts` (new `optionalDateBounds` — all-time when absent, calendar-invalid dates rejected to null via `isValidDateOnly`, end day-inclusive; From/To controls on `AnalyticsView`); `@typescript-eslint/unbound-method` promoted `warn → error` in all 3 eslint configs (0 violations). **Two PRs OPEN, intentionally held for owner/legal review — do NOT merge without sign-off, PRs #68/#69**: `POST /customers/:id/purge` GDPR/CCPA single-customer erasure (owner-gated, typed phone confirmation, atomic anonymize-in-place + audit_log PII redact, **runtime kill-switch `ENABLE_CUSTOMER_PURGE` — inert/404 until explicitly enabled**) and a disabled-by-default automated data-retention worker (`ENABLE_RETENTION_WORKER` + an explicit `RETENTION_DAYS`, no default window, anonymize-in-place). Both flagged because they erase PII irreversibly; broader-PII scope (voice_sessions/transcripts/appointment descriptions) is a deliberate follow-up. **Prod migrations: DONE 2026-06-23** — `20260622000000` (audit-extend) + `20260622010000` (requested_service_id) applied + verified on prod (`APPLIED=2`; `requested_service_id` column + `trg_audit_services`/`trg_audit_employees` triggers present; `schema_migrations` at `20260622010000`).
