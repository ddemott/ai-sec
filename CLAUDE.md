...
- `/supabase/migrations` — 148 SQL migrations (incl. 20260630 tenant_persona_name + 20260630 default_service_and_skill_backfill + 20260629 tenant_forwarded_from_phone + 20260625 job_inquiries + reap_stale_voice_sessions + 20260624 voice_session_caller_phone_nullable + 20260622 voice_session_requested_service + audit_services_employees + 20260619 fix_rls_gaps + 20260618 ai_cost_events + ai_cost_events_rls + 20260616 customer_messages + 20260615 preferences_default_true + 20260612 knowledge_suggestion + comms history + voice styles + forward phone). **`supabase/baseline.sql` is a generated pg_dump snapshot — regenerate with `npm run db:baseline` after any schema migration** (the schema-alignment guard fails CI when a migration-created table/column is missing from it).
