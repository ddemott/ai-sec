#!/usr/bin/env bash
# Restart script for the complete AI Secretary SaaS stack
# Kills existing processes on ports 3000 and 3001, then starts the stack.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")"/.. && pwd)"
cd "$ROOT_DIR"

echo "[ai-sec] 🔄 Restarting full stack..."

# 1. Kill any processes on our ports
echo "[ai-sec] 🧹 Cleaning up existing processes on ports 3000 and 3001..."
